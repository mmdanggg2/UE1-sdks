...
TCHAR GPackageInternal[64] = TEXT("Setup");
...
SQWORD FreeSpace( const TCHAR* Folder )
{
	...
	if( appStrlen(Folder) && appIsAlpha(Folder[0]) )
	{
		...
		GetDiskFreeSpaceW( Root, &SectorsPerCluster, &BytesPerSector, &FreeClusters, &TotalClusters );
...
void LocalizedFileError( const TCHAR* Key, const TCHAR* AdviceKey, const TCHAR* Filename )
{
	...
	appErrorf( NAME_FriendlyError, *FString::Printf(TEXT("%ls: %ls (%ls)\n\n%ls"),*LocalizeError(Key),Filename,appGetSystemErrorMessage(),*LocalizeError(AdviceKey)) );
	unguard;
}
...
class WFilerWizard : public WWizardDialog
{
	...
	void OnInitDialog()
	{
		...
		SendMessageW( *this, WM_SETICON, ICON_BIG, (WPARAM)LoadIconW(hInstance,MAKEINTRESOURCEW(IDICON_Setup1)) );
		//if( Manager->Logo==TEXT("") || !Manager->LocateSourceFile( Manager->Logo ) )
		{
			Manager->Logo = TEXT("..\\Help\\SetupLogo.bmp");//!!for setup
			if( GFileManager->FileSize(*Manager->Logo)<=0 )
				Manager->Logo = TEXT("SetupLogo.bmp");//!!for uninstaller
			if (GFileManager->FileSize(*Manager->Logo) <= 0)
				Manager->Logo = TEXT("..\\Help\\Logo.bmp");//!!fallback
		}
...
class WProductInfo : public WDialog
{
	...
	void LanguageChange()
	{
		...
		// General text.
		ProductText   .SetText(*Localize(TEXT("IDDIALOG_ProductInfo"),TEXT("IDC_ProductText")));
		VersionText   .SetText(*Localize(TEXT("IDDIALOG_ProductInfo"),TEXT("IDC_VersionText")));
		DeveloperText .SetText(*Localize(TEXT("IDDIALOG_ProductInfo"),TEXT("IDC_DeveloperText")));
		ProductHolder .SetText(*Localize(TEXT("IDDIALOG_ProductInfo"),TEXT("IDC_ProductHolder")));
...
class WFailedRequirement : public WDialog
{
	...
	void OnInitDialog()
	{
		...
		ProductInfo.ProductHolder.SetText( *Localize(TEXT("IDDIALOG_FailedRequirement"),TEXT("IDC_ProductHolder")) );
...
public:
	...
	, Forced            ( 0 )
	{
		...
		for( TArray<USetupGroup*>::TIterator It(SetupGroup->Subgroups); It; ++It )
			if( (*It)->Visible )
				Children.AddItem(new FComponentItem(OwnerComponents,*It,OwnerProperties,this) );
...
class WFilerPageProgress : public WWizardPage, public FInstallPoll
{
	...
	{
		return Finished ? WWizardPage::GetNextText() : TEXT("");
	}
};
...
class WFilerPagePreInstall : public WWizardPage
{
	...
	const FString GetNextText()
	{
		return *LocalizeGeneral(TEXT("InstallButton"),TEXT("Window"));
	}
	void OnInitDialog()
	{
		guard(WFilerPagePreInstall::OnInitDialog);
		WWizardPage::OnInitDialog();
		Message.SetText( *FString::Printf( *LineFormat(*LocalizeGeneral(TEXT("Ready"))), *Manager->LocalProduct, *Manager->DestPath, *Manager->LocalProduct ) );
		unguard;
	}
};
...
class WFilerPageComponents : public WFilerPageComponentsBase
{
	...
	void OnGroupChange( class FComponentItem* Group )
	{
		...
		SpaceAvailable.SetText( *FString::Printf(*LocalizeGeneral(TEXT("Space")), FreeSpace(*Manager->DestPath)/(1024*1024) ) );
		SpaceRequired .SetText( *FString::Printf(*LocalizeGeneral(TEXT("Space")), Manager->RequiredSpace/(1024*1024) ) );
	...
	WWizardPage* GetNext()
	{
		...
		if( FreeSpace(*Manager->DestPath) < Manager->RequiredSpace )
		{
			...
			MessageBox( *Owner, *FString::Printf(*LineFormat(*LocalizeGeneral(TEXT("NotEnoughSpace"))),Root,*Manager->LocalProduct), *LocalizeGeneral(TEXT("NotEnoughSpaceTitle")), MB_OK );
			return NULL;
		}
...
class WFilerPageFolder : public WWizardPage, public FControlSnoop
{
	...
	, BrowseButton		(this, IDC_Browse, FDelegate(this, (TDelegate)&WFilerPageFolder::OnBrowse))
	, Folder           ( this, IDC_Folder )
	, SpaceAvailable   ( this, IDC_SpaceAvailable )
	, SpaceRequired    ( this, IDC_SpaceRequired )
	{
	...
	void Update()
	{
		...
		SpaceAvailable.SetText( *FString::Printf(*LocalizeGeneral(TEXT("Space")), FreeSpace(*Folder.GetText())/(1024*1024) ) );
		SpaceRequired .SetText( *FString::Printf(*LocalizeGeneral(TEXT("Space")), Manager->RequiredSpace/(1024*1024) ) );
		unguard;
	}
	WWizardPage* GetNext()
	{
		...
		FString Title = FString::Printf( *LocalizeError(TEXT("FolderTitle") ), *NewFolder );
		FString Msg   = FString::Printf( *LocalizeError(TEXT("FolderFormat")), *NewFolder );
	...
	void OnInitDialog()
	{
		...
		WWizardPage::OnInitDialog();
		FolderDescription.SetText( *FString::Printf(*LineFormat(*Localize(TEXT("IDDIALOG_FilerPageFolder"),Manager->Patch ? TEXT("IDC_FolderDescriptionPatch") : TEXT("IDC_FolderDescription"))), *Manager->LocalProduct ) );
		OnReset();
	...
	void OnBrowse()
	{
		guard(WFilerPageFolder::OnBrowse);
		BROWSEINFO BrowseInfo = { 0 };
		BrowseInfo.lpszTitle = TEXT("Pick a Directory");
		LPITEMIDLIST Pidl = SHBrowseForFolder(&BrowseInfo);
		if (Pidl)
		{
			TCHAR Selected[MAX_PATH];
			if (SHGetPathFromIDList(Pidl, Selected))
...
class WFilerPageLicense : public WWizardPage
{
	...
	const FString GetNextText()
	{
		return *LocalizeGeneral(TEXT("AgreeButton"),TEXT("Window"));
	}
	WWizardPage* GetNext()
	{
...
class WFilerPageUninstall : public WWizardPage
{
	...
	void OnInitDialog()
	{
		...
		WWizardPage::OnInitDialog();
		SendMessageW( YesButton, BM_SETCHECK, 1, 0 );
		...
		SendMessageW( Prompt, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), MAKELPARAM(0,0) );
		FString Str;
	...
	WWizardPage* GetNext()
	{
		...
		if( SendMessageW( YesButton, BM_GETCHECK, 0, 0 )!=BST_CHECKED )
		{
			Owner->OnFinish();
			return NULL;
		}
...
class WFilerPageWelcome : public WWizardPage
{
	...
	void OnInitDialog()
	{
		...
		for( INT i=0; i<Results.Num(); i++ )
		{
			...
			GConfig->GetString( TEXT("Language"), TEXT("Language"), Name, *Path );
	...
	void OnUserChangeLanguage()
	{
		...
		{
			FString Language = TEXT("int"); //!!HACK
			UObject::SetLanguage( *Language );
			GConfig->SetString( TEXT("Setup"), TEXT("Language"), *Language, *Manager->ConfigFile );
		}
		LanguageChange();//!!
	...
	virtual void LanguageChange()
	{
		...
		// Welcome text.
		WelcomePrompt.SetText( *FString::Printf( *LineFormat(*Localize(TEXT("IDDIALOG_FilerPageWelcome"),Manager->Patch ? TEXT("IDC_WelcomePromptUpdate") : TEXT("IDC_WelcomePrompt") )), *Manager->LocalProduct, *Manager->Version ) );

		// Other text.
		Owner->SetText(*LineFormat(*Localize(TEXT("IDDIALOG_WizardDialog"), TEXT("IDC_WizardDialog") )));
		LanguagePrompt.SetText(*LineFormat(*Localize(TEXT("IDDIALOG_FilerPageWelcome"),TEXT("IDC_LanguagePrompt"))));
		ProductInfo.LanguageChange();//!!
		Owner->RefreshPage();//!!
...
class WFilerPageUninstallComponents : public WFilerPageComponentsBase
{
	...
	const FString GetNextText()
	{
		...
		return Dependencies.Num() ? WWizardPage::GetNextText() : TEXT("");
		unguard;
	}
	WWizardPage* GetNext()
	{
...
class WFilerPageAutoPlay : public WWizardPage
{
	...
	void OnPlay()
	{
		...
		if( Folder.Right(1)==PATH_SEPARATOR )
			Folder = Folder.LeftChop( 1 );
		ShellExecuteW( *this, TEXT("open"), *Exe, TEXT(""), *Folder, SW_SHOWNORMAL );
		Owner->OnFinish();
		unguard;
	}
	void OnInstall()
	...
	void OnUninstall()
	{
		...
		ShellExecuteW( NULL, TEXT("open"), *(Path*TEXT("Setup.exe")), *(US+TEXT("uninstall \"") + Manager->Product + TEXT("\"")), *Path, SW_SHOWNORMAL );
		Owner->OnFinish();
		unguard;
	}
	void OnReleaseNotes()
	{
		...
		ShellExecuteW( *this, TEXT("open"), *(Manager->RegistryFolder * Manager->ReadMe), TEXT(""), NULL, SW_SHOWNORMAL );
		unguard;
	}
	void OnWeb()
	{
		guard(WFilerPageAutoPlay::OnWeb);
		ShellExecuteW( *this, TEXT("open"), *Manager->ProductURL, TEXT(""), appBaseDir(), SW_SHOWNORMAL );
		unguard;
	}
	...
	void OnInitDialog()
	{
		...
		if( Manager->MustReboot )
			CompleteLabel.SetText( *LineFormat(*Localize(TEXT("IDDIALOG_FilerPageAutoPlay"),TEXT("IDC_CompleteReboot"))) );
	...
	const FString GetFinishText()
	{
		return ShowInstallOptions ? TEXT("") : Manager->MustReboot ? *LocalizeGeneral(TEXT("RebootButton")) : *LocalizeGeneral(TEXT("FinishButton"), TEXT("Window"));
	}
	const FString GetNextText()
	{
		return (Manager->MustReboot && !ShowInstallOptions) ? *LocalizeGeneral(TEXT("ExitButton")) : TEXT("");
	}
	virtual const FString GetCancelText()
	{
		return ShowInstallOptions ? WWizardPage::GetCancelText() : TEXT("");
	}
};
...
INT WINAPI WinMain( HINSTANCE hInInstance, HINSTANCE hPrevInstance, char* InCmdLine, INT nCmdShow )
{
...
#endif
	{
		...
		INT Count=0;
	RetryMutex:
		hMutex = CreateMutexW( NULL, 0, TEXT("UnrealIsRunning") );
