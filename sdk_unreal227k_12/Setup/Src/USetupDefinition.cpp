	...
	if( Selected )
	{
		...
		for( TArray<FString>::TIterator ItF(File); ItF; ++ItF )
		{
			...
			if( Info.Lang==TEXT("") || Info.Lang==UObject::GetLanguage() )
			{
				...
				INT NewSize = (Info.Src.Right(3).Caps()==TEXT(".UZ")) ? Info.Size*3 : Info.Size;
				Count += (ExistingSize<0) ? (NewSize + PER_FILE_OVERHEAD) : Max<SQWORD>(0,NewSize-ExistingSize);
			}
		}
	}
	return Count;
...
/*-----------------------------------------------------------------------------
	FDecompressSetupNotify.
-----------------------------------------------------------------------------*/
class FDecompressNotify : public FCodecNotify
{
public:
	USetupDefinition* Definition;
	FInstallPoll* Poll;
	FString Filename;
	INT Size;
	INT OldProgress;
	...
	{
		if( !OldSize || (INT)(Progress*100) != OldProgress )
		{
			INT NowSize = Progress*Size;
			Definition->RunningBytes += (NowSize - OldSize);
			OldSize = NowSize;
			OldProgress = (INT)(Progress*100);
			return Poll->Poll(*Filename,NowSize,Size,Definition->RunningBytes,Definition->TotalBytes);
		}
		return 1;
	}
};

FDecompressNotify DecompressNotify;
...
void USetupDefinition::Init()
{
	...
	BOOL bInstall = Token==TEXT("install") || GFileManager->FileSize(*Token) > 0;
	if( bInstall || (Token==TEXT("") && MasterProduct==TEXT("")))
	{
		// Installing.
		if( bInstall )
		{
			...
			// stijn: The System/Manifest.int file always took precedence over the Manifest.int file embedded in the umod because this flush was missing
			GConfig->Flush(1, *(FString(MANIFEST_FILE) + TEXT(".") + UObject::GetLanguage()));
			LoadConfig();
			LoadLocalized();
		}
	...
	if( !Exists && !Uninstalling && !NoRun )
	{
		...
		for( TArray<FString>::TIterator It(Requires); It; ++It )
			...
			{
				if( RequiredProduct==TEXT("Unreal") )
					RequiredProduct = TEXT("Unreal");
...
{
	if( RefFile.Left(7)==FString(TEXT("System") PATH_SEPARATOR) )  //oldver for UT GOTY version, general multi-reference-path solution?
	{
		FString Test = RefPath * FString(TEXT("System400") PATH_SEPARATOR) + RefFile.Mid(7);
...
{
	guard(DidCancel);
	appMsgf( *LocalizeGeneral(TEXT("DidCancel"))), *LocalizeGeneral(TEXT("InstallCancel") );
	unguard;
}
void USetupDefinition::InstallTree( const TCHAR* Action, FInstallPoll* Poll, void (USetupDefinition::*Process)( FString Key, FString Value, UBOOL Selected, FInstallPoll* Poll ), USetupGroup* SetupGroup, UBOOL Selected )
{
	...
	else appMsgf(TEXT("SetupGroup could not get %ls from %ls"), SetupGroup->GetName(), *ConfigFile);
	unguardf(( TEXT("(%ls %ls)"), Action, SetupGroup->GetFullName() ));
}
...
void USetupDefinition::BeginSteps()
{
	...
	if( Uninstalling )//!!can merge?
	{
		for( TArray<USetupGroup*>::TIterator GroupIt(UninstallComponents); GroupIt; ++GroupIt )
		{
			TMultiMap<FString,FString>* cMap = GConfig->GetSectionPrivate( (*GroupIt)->GetName(), 0, 1, *SetupIniFile );
			if( cMap )
				(*GroupIt)->UninstallLog = *cMap;
		}
	}
	else
	{
		TMultiMap<FString,FString>* cMap = GConfig->GetSectionPrivate( *Product, 0, 1, *SetupIniFile );
		if( cMap )
			RootGroup->UninstallLog = *cMap;
	}
...
{
	guard(USetupDefinition::DoUninstallSteps);
...
UBOOL USetupDefinition::CheckRequirement( FString Folder, USetupProduct* RequiredProduct, FString& FailMessage )
{
	...
	if( R==TEXT("Unreal") )
		R = TEXT("Unreal");
	//appMsgf(TEXT("%ls %ls %ls %ls"),*RequiredProduct->MapCheck,*RequiredProduct->DLLCheck,*RequiredProduct->UCheck,*RequiredProduct->TextureCheck);
	if( !GConfig->GetString( *R, TEXT("Version"), ExistingVersion, *(Folder + PATH_SEPARATOR TEXT("System") PATH_SEPARATOR SETUP_INI) ) )
	{
		...
		if(	RequiredProduct->OldVersionInstallCheck!=TEXT("")&&	GFileManager->FileSize(*(Folder * RequiredProduct->OldVersionInstallCheck))>0 )
		{
			// Old version found.
	...
	if(	RequiredProduct->MapCheck!=TEXT("") && !(GFileManager->FileSize(*(Folder * RequiredProduct->MapCheck))>0) )
	{
		FailMessage = FString::Printf( *LocalizeError(TEXT("MissingProduct")), Patch ? *LocalizeError(TEXT("MissingProductThis")) : *LocalProduct, *RequiredProduct->Product, *RequiredProduct->Product );
		return 0;
	}
	if(	RequiredProduct->DLLCheck!=TEXT("") && !(GFileManager->FileSize(*(Folder * RequiredProduct->DLLCheck))>0) )
	{
		FailMessage = FString::Printf( *LocalizeError(TEXT("MissingProduct")), Patch ? *LocalizeError(TEXT("MissingProductThis")) : *LocalProduct, *RequiredProduct->Product, *RequiredProduct->Product );
		return 0;
	}
	if(	RequiredProduct->TextureCheck!=TEXT("") && !(GFileManager->FileSize(*(Folder * RequiredProduct->TextureCheck))>0) )
	{
		FailMessage = FString::Printf( *LocalizeError(TEXT("MissingProduct")), Patch ? *LocalizeError(TEXT("MissingProductThis")) : *LocalProduct, *RequiredProduct->Product, *RequiredProduct->Product );
		return 0;
	}
	if(	RequiredProduct->UCheck!=TEXT("") && !(GFileManager->FileSize(*(Folder * RequiredProduct->UCheck))>0) )
	{
		FailMessage = FString::Printf( *LocalizeError(TEXT("MissingProduct")), Patch ? *LocalizeError(TEXT("MissingProductThis")) : *LocalProduct, *RequiredProduct->Product, *RequiredProduct->Product );
		return 0;
	}
...
void USetupDefinition::ProcessCopy( FString Key, FString Value, UBOOL Selected, FInstallPoll* Poll )
{
	...
	if( Selected && Key==TEXT("File") )
	{
		...
		if( Info.Lang==TEXT("") || Info.Lang==UObject::GetLanguage() )
		{
			...
			debugf( TEXT("Copying %ls to %ls"), *FullSrc, *ThisDest);
				...
				*SrcAr << Signature;
				...
				else
				{
					*SrcAr << OrigFilename;
					...
					DecompressNotify.Definition = this;
					DecompressNotify.Poll = Poll;
					DecompressNotify.Filename = FullDest;
					DecompressNotify.Size = Size;
					DecompressNotify.OldProgress = 0;
					DecompressNotify.OldSize = 0;

					if( !Codec.Decode( *SrcAr, *DestAr, &DecompressNotify ) )
					{
						delete SrcAr;
						delete DestAr;
						DidCancel();
					}
					RunningBytes += (Size - DecompressNotify.OldSize);
				}
			}
			else
			{
			...
			if( Info.Ref!=TEXT("") )
			{
				...
				debugf( TEXT("Patching %ls to %ls with %ls"), *ThisSrc, *FullDest, *DeltaName );
				...
				// Validate.
				if( Magic!=0x92f92912 )
					appErrorf( LineFormat(*LocalizeError(TEXT("PatchCorrupt"))), *DeltaName, *LocalizeError("AdviseBadDownload") );
				if( OldSize!=Size || OldCRC!=CalcOldCRC )
					appErrorf( *LocalizeError(TEXT("CdFileMismatch")), *Info.Ref, *LocalProduct );
				...
				while( !Reader.AtEnd() )
				{
					...
					else
					{
						...
						SrcAr->Seek( CopyPos );
						for( INT Base=Index; Base>0; Base-=sizeof(wBuffer) )
						{
							INT Move = Min(Base,(INT)sizeof(wBuffer));
							SrcAr->Serialize( wBuffer, Move );
							if( SrcAr->IsError() )
								LocalizedFileError( TEXT("FailedReadingSource"), Patch ? TEXT("AdviseBadDownload") : TEXT("AdviseBadDownload"), *ThisSrc );
							CRC = appMemCrc( wBuffer, Move, CRC );
							DestAr->Serialize( wBuffer, Move );
							if( DestAr->IsError() )
								LocalizedFileError( TEXT("FailedWritingDest"), TEXT("AdviseBadDest"), *FullDest );
						}
						CountSize += Index;
					...
					if( ((CountSize^OldCountSize)&~(sizeof(wBuffer)-1)) || Reader.AtEnd() )
					{
				...
				if( NewSize!=CountSize || NewCRC!=CRC )
					appErrorf( LineFormat(*LocalizeError(TEXT("PatchCorrupt"))), *DeltaName, *LocalizeError(TEXT("AdviseBadDownload")) );
				delete SrcAr;
				if( !DestAr->Close() )
...
void USetupDefinition::ProcessExtra( FString Key, FString Value, UBOOL Selected, FInstallPoll* Poll )
{
	...
	else if ( Key==TEXT("ClearIniSec") )
    {
         Value            = Format(Value,NULL);
         INT Pos          = Value.InStr(TEXT(","));
         check(Pos>=0);
         FString IniFile  = DestPath*Value.Left(Pos);
         FString IniSec   = Value.Mid(Pos+1);
         GConfig->EmptySection(*IniSec, *IniFile);              
    }
	unguard;
}
void USetupDefinition::ProcessPostCopy( FString Key, FString Value, UBOOL Selected, FInstallPoll* Poll )
{
	...
	if( Key==TEXT("AddIni") && Selected )
	{
		...
		if( IniFile.Caps() != TEXT("UNREAL.INI") )
