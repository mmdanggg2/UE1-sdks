...
#else
TArray<FString> FindFilesRecursive( FString Path, FString Spec )
#endif
{
...
struct FFileInfo
{
	...
	void Write( FOutputDevice& Ar, UBOOL Distribution )
	{
		...
		Ar.Logf( TEXT("(Src=%ls"), *Safe(Src) );
		if( Dest!=TEXT("") )
			Ar.Logf( TEXT(",Dest=%ls"), *Safe(Dest) );
		if( Distribution && Master!=TEXT("") )
			Ar.Logf( TEXT(",Master=%ls"), *Safe(Master) );
		if( Ref!=TEXT("") )
			Ar.Logf( TEXT(",Ref=%ls"), *Safe(Ref) );
		if( RefSize )
			Ar.Logf( TEXT(",RefSize=%i"), RefSize );
		if( Lang!=TEXT("") )
			Ar.Logf( TEXT(",Lang=%ls"), *Lang );
		if( Size!=0 )
...
class USetupProduct : public USetupObject
{
	...
	FStringNoInit OldVersionInstallCheck, OldVersionNumber,MapCheck,DLLCheck,UCheck,TextureCheck; //oldver Per install.
	...
	void StaticConstructor()
	{
		...
		new(GetClass(),TEXT("MapCheck"		        ),RF_Public)UStrProperty(CPP_PROPERTY(MapCheck	            ), TEXT(""), CPF_Config);
		new(GetClass(),TEXT("DLLCheck"		        ),RF_Public)UStrProperty(CPP_PROPERTY(DLLCheck	            ), TEXT(""), CPF_Config);
		new(GetClass(),TEXT("UCheck"				),RF_Public)UStrProperty(CPP_PROPERTY(UCheck	            ), TEXT(""), CPF_Config);
		new(GetClass(),TEXT("TextureCheck"			),RF_Public)UStrProperty(CPP_PROPERTY(TextureCheck          ), TEXT(""), CPF_Config);
...
class USetupGroup : public USetupObject
{
	...
	void StaticConstructor()
	{
		guard(USetupGroup::StaticConstructor);
		UArrayProperty* P;
