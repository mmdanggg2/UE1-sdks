...
struct FWindowsCursor : public FCursorData
{
private:
...
public:
	UBOOL bInvalid;
	FWindowsCursor(UUnrealCursor* C)
	{
		guard(FWindowsCursor::FWindowsCursor);
		TArray<FAnimCursorData> Frames;
		FTime CTime = appSeconds();
		for (INT i = 0; i < C->Frames.Num(); ++i)
		{
			UTexture* T = C->Frames(i);
			if (!T)
				continue;
			FTextureInfo* Info = T->GetTexture(INDEX_NONE, NULL);
			if (Info && Info->Format == TEXF_P8)
			{
				new (Frames) FAnimCursorData(Info->Mips[0]->DataPtr, Info->USize, Info->VSize, Info->UClamp, Info->VClamp, C->HotSpotX, C->HotSpotY, Info->Palette);
			}
		}

		bInvalid = !Frames.Num() || !Cursor.CreateCursorTex(&Frames(0), Frames.Num(), C->AnimRate);
		unguard;
	}
	~FWindowsCursor() noexcept(false)
	{}
...
UBOOL UWindowsViewport::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	{
		Ar.Log( *RenDev->GetClass()->GetPathNameSafe() );
		return 1;
	}
	else if( ParseCommand(&Cmd,TEXT("GetRes")) )
	{
		...
		{
			// Windowed mode.
			Ar.Log( TEXT("320x240 400x300 512x384 640x480 800x600 1024x768 1280x1024") );
		}
		return 1;
	}
	...
	else if( ParseCommand(&Cmd,TEXT("Preferences")) )
	{
		...
		{
			Client->ConfigProperties = new WConfigProperties(TEXT("Preferences"), *LocalizeGeneral("AdvancedOptionsTitle", TEXT("Window"), TEXT("int")), *LocalizeGeneral("AdvancedOptionsTitle", TEXT("Window")));
	...
	else if (ParseCommand(&Cmd, TEXT("SAVESCREENPOS")))
	{
		if (!GIsEditor && !IsFullscreen() && !ParentWindow && Window && Window->hWnd)
		{
			...
			if (C && wr.Min.X != -32000 && wr.Min.Y != -32000 && ((C->WinPosX != wr.Min.X) || (C->WinPosY != wr.Min.Y)))
				{
					C->WinPosX = wr.Min.X;
					C->WinPosY = wr.Min.Y;
...
void UWindowsViewport::OpenWindow( void* InParentWindow, UBOOL IsTemporary, INT NewX, INT NewY, INT OpenX, INT OpenY, const TCHAR* ForcedRenDevClass )
{
	...
	else
	{
		...
		else
		{
			...
			if (OpenX == -1)
			{
				// CheckMe!!! validate WinPosX/Y, sometimes insane values like -32000 are set for some reason.
				C->WinPosX = Max(0, C->WinPosX);
				C->WinPosY = Max(0, C->WinPosY);
	...
	if (!RenDev && (BlitFlags & BLIT_Temporary))
	{
		...
		RenDev = ConstructObject<URenderDevice>(URenderDevice::GetNullRenderDevice(), this);
		check(RenDev);
	}
#if _DEBUG
	if (!RenDev && !GIsEditor)
		TryRenderDevice(TEXT("ini:Engine.Engine.GameRenderDevice"), NewX, NewY, INDEX_NONE, FALSE); // Marco: Never start debug with fullscreen or window will freeze if it encounters exception!
#endif
	...
	if (DoSetActive)
	{
		if (!GIsEditor && !IsFullscreen() && !ParentWindow)
		{
			RECT desktop, winsz;
...
void UWindowsViewport::SetModeCursor()
{
	...
	HCURSOR hCursor;
	BYTE CursorMode = OutputDev->GetCursor();
	if (CursorMode < EM_CustomCursor)
	{
		switch (CursorMode)
		{
	...
	else if (CursorMode < (EM_CustomCursor + ARRAY_COUNT(StandardCursors)))
		hCursor = StandardCursors[CursorMode - EM_CustomCursor];
	else hCursor = LoadCursorW(NULL, IDC_ARROW);
...
void UWindowsViewport::UpdateWindowFrame()
{
	...
	Window->SetText(OutputDev->GetWindowName());
...
UBOOL UWindowsViewport::CauseInputEvent( INT iKey, EInputAction Action, FLOAT Delta )
{
	...
	if( iKey>=0 && iKey<IK_MAX )
		return OutputDev->InputEvent((EInputKey)iKey, Action, Delta);
	else
		return 0;
...
// Update mouse pointer icon.
void UWindowsViewport::UpdateMouseCursor()
{
	guard(UWindowsViewport::UpdateMouseCursor);
	if (PrevCursor != CustomCursor)
	{
		PrevCursor = CustomCursor;
		if (CustomCursor)
		{
			if (!CustomCursor->CursorData)
				CustomCursor->CursorData = new FWindowsCursor(CustomCursor);
			if (((FWindowsCursor*)CustomCursor->CursorData)->bInvalid)
			{
				CustomCursorIc = NULL;
				SelectedCursor = CustomCursor->FallbackCursor;
			}
			else CustomCursorIc = ((FWindowsCursor*)CustomCursor->CursorData)->GetCursor();
		}
	}
	if (CustomCursor)
	{
		if (CustomCursorIc)
			SetCursor(CustomCursorIc);
		else SetCursor(StandardCursors[Min<BYTE>(CustomCursor->FallbackCursor, 6)]);
	}
	else SetCursor(StandardCursors[Min<BYTE>(SelectedCursor, 6)]);
	unguard;
}
	...
	switch( iMessage )
	{
		...
		case WM_PAINT:
		{
			...
			else if( IsWindowVisible(Window->hWnd) && SizeX && SizeY && hBitmap )
			{
				...
				if( BitBlt( hDC, 0, 0, SizeX, SizeY, Client->hMemScreenDC, 0, 0, SRCCOPY ) == NULL )
					appErrorf( TEXT("BitBlt failed: %ls"), appGetSystemErrorMessage() );
		...
		case WM_COMMAND:
		{
      		...
      		switch( LOWORD(wParam) )
			{
				...
				{
					Actor->RendMap=REN_LightingOnly;
					break;
				}
				case ID_Normals:
				{
					Actor->RendMap = REN_Normals;
					break;
				}
				case ID_Win320:
				{
				...
				{
					Actor->ShowFlags ^= SHOW_RealTimeBackdrop;
					break;
				}
		...
		case WM_CHAR:
		{
			guard(WM_CHAR);
			EInputKey Key = (EInputKey)wParam;
			if (Key != IK_Enter && OutputDev->Key(Key, (INT)wParam))
			{
				// Redraw if needed.
				RepaintPending = TRUE;
		...
		case WM_LBUTTONDBLCLK:
		{
			...
			{
				OutputDev->Click(MOUSE_LeftDouble, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
				RepaintPending = TRUE;
			}
			return 0;
		}
		...
		case WM_DISPLAYCHANGE:
		{
			...
			debugf( NAME_Log, TEXT("Viewport %ls: WM_DisplayChange"), GetName() );
			unguard;
			return 0;
		}
...
UBOOL UWindowsViewport::ResizeViewport( DWORD NewBlitFlags, INT InNewX, INT InNewY, INT InNewColorBytes )
{
	...
	else if( !(NewBlitFlags & BLIT_Temporary) && !(NewBlitFlags & BLIT_NoWindowChange) )
	{
		...
		{
			HoldCount++;
			SetWindowLongPtrW( Window->hWnd, GWL_STYLE, GetWindowLongPtrW(Window->hWnd,GWL_STYLE) | (WS_CAPTION|WS_THICKFRAME) );
			SetWindowPos(Window->hWnd, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED); // Regarding MSDN: If you have changed certain window data using SetWindowLong, you must call SetWindowPos for the changes to take effect. Use the following combination for uFlags: SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED.
			HoldCount--;
		}
...
UBOOL UWindowsViewport::FireWallHack(INT Cmd)
{
	...
	{
		GWarn->Logf(TEXT("FIREWALL: Could not Create Windows FireWall Object"));
		return true;
	}
	...
	if ( SUCCEEDED( hr=fwMgr->get_LocalPolicy(&fwPolicy)) )
	{
		...
		if ( SUCCEEDED(hr = fwPolicy->get_CurrentProfile(&fwProfile) ) )
		{
			...
			if ( SUCCEEDED( hr=fwProfile->get_FirewallEnabled(&fwEnabled)) && (fwEnabled == VARIANT_TRUE) )
			{
					...
					if (SysStringLen(fwBstrProcessImageFileName) >0)
					{
						...
						if (Cmd==0)	// Check to See if it already authorized
						{
							if ( SUCCEEDED(hr = fwApps->Item(fwBstrProcessImageFileName,&fwApp)) )
							{
								...
								{
									GWarn->Logf(TEXT("FIREWALL: Unreal is not authorized for outside access"));
									Result = FALSE;
								}
							...
							{
								GWarn->Logf(TEXT("FIREWALL: Unreal status is undefined therefore not authorized for outside access"));
								Result = FALSE;
							}
						}
				...
				GWarn->Logf(TEXT("FIREWALL: Windows FireWall Not Enabled"));
	...
	fwMgr->Release();
	GWarn->Logf(TEXT("FIREWALL: Freed Windows Firewall Object"));
