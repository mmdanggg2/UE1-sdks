...
void UWindowsClient::Init( UEngine* InEngine )
{
	...
	if( !hMemScreenDC )
		appErrorf( TEXT("CreateCompatibleDC failed: %ls"), appGetSystemErrorMessage() );
	...
	for( ; ; )
	{
		...
		debugf(NAME_DevGraphics, TEXT("Initializing DirectDraw") );
		...
		DirectDraws.Empty();
		debugf(NAME_DevGraphics, TEXT("DirectDraw drivers detected:"));
		...
		{
			debugf(NAME_Init, TEXT("DirectDraw created failed: %ls"), ddError(Result));
			break;
		}
		unguard;		
		...
		if (!FName::SafeSuppressed(NAME_DevGraphics))
		{
			debugf(NAME_DevGraphics, TEXT("DirectDraw: Enumerated Display Modes:"));
		for (TArray<DDSURFACEDESC2>::TIterator It(DisplayModes); It; ++It)
				debugf(NAME_DevGraphics, TEXT("DirectDraw: Mode %dx%dx%d"), It->dwWidth, It->dwHeight, It->ddpfPixelFormat.dwRGBBitCount);
		}
		unguard;
...
void UWindowsClient::Destroy()
{
	...
	if (dd)
	{
		...
		if (Result != DD_OK)
			debugf(NAME_Exit, TEXT("DirectDraw Release failed: %ls"), ddError(Result));
		else
...
void UWindowsClient::ddEndMode()
{
	...
	if( dd )
	{
		...
		if( Result!=DD_OK )
			debugf( NAME_Log, TEXT("DirectDraw SetCooperativeLevel: %ls"), ddError(Result) );

		// Restore the display mode.
		Result = dd->RestoreDisplayMode();
		if( Result!=DD_OK )
			debugf( NAME_Log, TEXT("DirectDraw RestoreDisplayMode: %ls"), ddError(Result) );
...
void UWindowsClient::PostEditChange()
{
	...
	if( UseJoystick && !ParseParam(appCmdLine(),TEXT("NoJoy")) && !GIsEditor )
	{
		...
		if( joyGetDevCapsA(JOYSTICKID1,&JoyCaps,sizeof(JoyCaps))==JOYERR_NOERROR )
			debugf( TEXT("Detected joysticks: %i (%ls)"), nJoys, JoyCaps.szOEMVxD ? appFromAnsi(JoyCaps.szOEMVxD) : TEXT("None") );
...
UBOOL UWindowsClient::SetClipboardText(FString& Str)
{
	...
	if (OpenClipboard(GetActiveWindow()))
	{
		...
		UBOOL Result = SetClipboardData(CF_UNICODETEXT, Data) && CloseClipboard();
		return Result;
	}
	return FALSE;
	unguard;
...
FString UWindowsClient::GetClipboardText()
{
	...
	if (OpenClipboard(GetActiveWindow()))
	{
		...
		{
			WCHAR* pchData = (WCHAR*)GlobalLock(V);
			if (pchData)
			{
				Result = reinterpret_cast<TCHAR*>(V);
				GlobalUnlock(V);
			}
		}
		else if ((V = GetClipboardData(CF_TEXT)) != nullptr)
		{
			Result = reinterpret_cast<ANSICHAR*>(V);
		}
