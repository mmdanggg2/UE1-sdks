	...
	switch (iMessage)
	{
		...
		case WM_XBUTTONDOWN:
		{
			...
			if (State)
			{
				...
				State->LastClickTime = appSeconds();
...
void FWindowsMouseInputHandler::ProcessInputUpdates(UWindowsViewport* Viewport)
{
	...
	if (WheelMoved)
	{
		...
		if (GIsEditor)
		{
			PostMessageW(Viewport->ParentWindow, WM_VSCROLL, MAKEWPARAM((WheelDown ? SB_LINEDOWN : SB_LINEUP), 0), WM_MOUSEWHEEL);// Hack to make scrollbar working in TextureBrowser.
			Viewport->GetOuterUWindowsClient()->Engine->MouseDelta(Viewport, WheelDown ? MOUSE_WheelDown : MOUSE_WheelUp, 0, 0); // in ortho viewports use it for zoom
		}
		...
		for (INT i = 0; i < NumButtons; ++i)
		{
			if (ButtonStates[i].IsDown && !ButtonStates[i].WasDown)
			{
				...
				if (GIsEditor && ButtonStates[i].HaveLastClickLocation)
				{
					...
					Viewport->GetOutputAPI()->edSetClickLocation(V);
					...
					{
						Viewport->GetOutputAPI()->Click(MOUSE_Left, static_cast<FLOAT>(ButtonStates[i].LastClickLocation.x), static_cast<FLOAT>(ButtonStates[i].LastClickLocation.y));
						DeliveredClick = TRUE;
					}
				}
			}
			else if (ButtonStates[i].WasDown && !ButtonStates[i].IsDown)
			{
				...
				if (GIsEditor)
				{
					...
					if (!(Viewport->BlitFlags & BLIT_Fullscreen) &&
						...
						Abs<FLOAT>(ButtonStates[i].LastClickTime - appSeconds()) <= Viewport->ClickDetectionTimeThreshold &&
							...
							|| (!LeftButtonDown && !RightButtonDown)))
					{
						...
						Viewport->GetOutputAPI()->Click(ButtonStates[i].ViewportButtonFlag, MouseX, MouseY);
	...
	if (UpdateViewport)
	{
		...
		if (!Viewport->CapturingMouse)
		{
				...
				ClientPosition.y >= TempRect.top && ClientPosition.y < TempRect.bottom)
			{
				...
				const FLOAT mDX = static_cast<FLOAT>(ClientPosition.x - TempRect.left);
				const FLOAT mDY = static_cast<FLOAT>(ClientPosition.y - TempRect.top);
				Viewport->GetOutputAPI()->MousePosition(ViewportButtonFlags, mDX, mDY);
			}
		...
		else if (CursorMoved)
		{
			...
			Viewport->GetOutputAPI()->MouseDelta(ViewportButtonFlags, DX, DY);
		...
		if (UpdateViewport)
		{
			...
			if (!Viewport->CapturingMouse && Viewport->bShowWindowsMouse)
...
void FWindowsWin32InputHandler::PollInputs(UWindowsViewport* Viewport)
{
	...
	{
		CursorMoved = TRUE;
		const LONG Cumulative = Abs<LONG>(NewCursorPos.x - OldCursorPos.x) + Abs<LONG>(NewCursorPos.y - OldCursorPos.y);
		for (INT i = 0; i < NumButtons; ++i)
...
UBOOL FWindowsDirectInputHandler::SetupInput()
{
	...
	if (Result != DI_OK)
	{
		debugf(NAME_DevInput, TEXT("DirectInput created failed: %ls"), diError(Result));
		ShutdownInput();
		return FALSE;
	}
...
void FWindowsDirectInputHandler::PollInputs(UWindowsViewport* Viewport)
	...
	{
		CursorMoved = TRUE;
		const LONG Cumulative = Abs<LONG>(DX) + Abs<LONG>(DY);
		for (INT i = 0; i < NumButtons; ++i)
			ButtonStates[i].CumulativeMovementSinceLastClick += Cumulative;
	}
