...
#define MAX_TEX_SIZE 2048 // stijn: increased to 2048 for UTv469
...
#define LoadLibraryX(a) LoadLibraryW(a)
...
//
class DLL_EXPORT_CLASS UD3DRenderDevice : public URenderDevice
{
	...
	// stijn: uploads a white texture. Used in place of NPOT textures
	struct FTexFiller8888_Empty : public FTexFiller
	{
		FTexFiller8888_Empty( UD3DRenderDevice* InOuter ) : FTexFiller(&InOuter->Format8888) {}
		void UploadMipmap( UD3DRenderDevice* Device, FTexInfo* ti, FRainbowPtr Dest, INT Stride, const FTextureInfo& Info, INT MipIndex, DWORD PolyFlags )
		{
			guard(FTexFiller8888_Empty::UploadMipmap);
			INT  USize      = 1 << Info.Mips[MipIndex]->UBits;
			INT  VSize      = 1 << Info.Mips[MipIndex]->VBits;
			if( ti->UIndex==0 )
				for( INT j=VSize-1; j>=0; j--,Dest.PtrBYTE+=Stride-USize*sizeof(DWORD) )
					for( INT k=USize-1; k>=0; k--,Dest.PtrDWORD++ )
						*Dest.PtrDWORD = AlphaPalette[255];
			else
				for( INT j=VSize-1; j>=0; j--,Dest.PtrBYTE+=sizeof(DWORD)-Stride*USize )
					for( INT k=USize-1; k>=0; k--,Dest.PtrBYTE+=Stride )
						*Dest.PtrDWORD = AlphaPalette[255];
		...
		void BeginUpload( UD3DRenderDevice* Device, FTexInfo* ti, const FTextureInfo& Info, DWORD PolyFlags )
		{
			guard(FTexFiller8888_Empty::BeginUpload);
			for( INT i=0; i<NUM_PAL_COLORS; i++ )
				AlphaPalette[i] = RGBA_MAKE(i,i,i,i);
			if( PolyFlags & PF_Masked )
				AlphaPalette[0] = 0;
			unguard;
		}
	...
	FTexFiller8888_Empty	FillerNPOT;
    ...
    void StaticConstructor()
	{
		...
		MaxResWidth				= 2048; // stijn: turns out that D3D7 is capped at 2048px resolutions... silly
		MaxResHeight			= 2048;
	...
	UBOOL Init( UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )
	{
		...
		debugf(TEXT("UseVSync: %i"),UseVSync);
	...
	void Flush( UBOOL AllowPrecache )
	{
		...
		if( Direct3DDevice7 )
		{
			...
			for( INT i=0; i<ARRAY_COUNT(CachedTextures); i++ )
	...
	void Lock( FPlane InFlashScale, FPlane InFlashFog, FPlane ScreenClear, DWORD InLockFlags, BYTE* InHitData, INT* InHitSize )
	{
		...
		if( hr!=DD_OK )
		{
			debugf(TEXT("TestCooperativeLevel failed (%ls)"),*D3DError(hr));
			Failed:
			...
			debugf(TEXT("Resetting mode (%ls)"),*D3DError(hr2));
		...
		if( FAILED(h=Direct3DDevice7->BeginScene()) )
		{
			if( ++FailCount==1 )
				goto Failed;
			appErrorf(TEXT("BeginScene failed (%ls)"),*D3DError(h));
		}
		unguard;
	...
	void DrawComplexSurface( FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet )
	{
		...
		else
		{
			...
			INT First = VertsUWindow.Unlock();
			for(FSavedPoly* Poly=Facet.Polys; Poly; n+=Poly->NumPts,Poly=Poly->Next)
			{
				Direct3DDevice7->DrawPrimitiveVB( D3DPT_TRIANGLEFAN, VertsUWindow.VertexBuffer7, First, Poly->NumPts, 0 );
				First += Poly->NumPts;
				Stats.Polys++;
		...
		if( IsSelected )
		{
			...
			if (Surface.PolyFlags & PF_FlatShaded)
				...
				if (Surface.PolyFlags & PF_Selected)
					polyColor = RGBA_MAKE((Surface.FlatColor.R * 2), (Surface.FlatColor.G * 2), (Surface.FlatColor.B * 2), 255);
				else
					polyColor = RGBA_MAKE(Surface.FlatColor.R, Surface.FlatColor.G, Surface.FlatColor.B, 255);
				}

			else
				polyColor = RGBA_MAKE( 0,0,0,0);
	...
	void DrawGouraudPolygon( FSceneNode* Frame, FTextureInfo& Info, FTransTexture** Pts, INT NumPts, DWORD PolyFlags, FSpanBuffer* Span )
	{
		...
		// stijn: we don't do clipping in software anymore except for rendevs that really need it
	...
	UBOOL Exec( const TCHAR* Cmd, FOutputDevice& Ar )
	{
		...
		else if( ParseCommand(&Cmd, TEXT("ShowPools")) )
		{
			...
			for( FPixFormat* Fmt=FirstPixelFormat; Fmt; Fmt=Fmt->Next )
			{
				Ar.Logf(TEXT(" PixFormat: %ls"), Fmt->Desc );
				for( TMap<INT,FTexPool*>::TIterator ItP(Fmt->PoolList); ItP; ++ItP )
	...
	void UD3DRenderDevice::PushHit( const BYTE* Data, INT Count )
	{
		...
		if( ddrval!=DD_OK )
		{
			if( ddrval!=DDERR_SURFACELOST )
				debugf( NAME_Log, TEXT("D3D Driver: Hit-detection backbuffer lock failed (%ls)"), *D3DError(ddrval) );
			return;
		}
	...
	void GetStats( TCHAR* Result )
	{
		...
		//Frame->Viewport->Canvas->WrappedPrintf( Frame->Viewport->Canvas->SmallFont, 0, TEXT(" %ls\n"), TempStr );
	...
	void ReadPixels(FColor* Pixels, UBOOL bGammaCorrectOutput)
	{
		...
		DECLARE_INITED(DDSURFACEDESC2,ddsd);
		// stijn: on Win10, we cannot read from the primary surface in windowed mode...
		IDirectDrawSurface7* Surf = ViewportFullscreen ? PrimarySurface7 : BackSurface7;
		HRESULT ddrval = Surf->Lock( NULL, &ddsd, DDLOCK_READONLY|DDLOCK_SURFACEMEMORYPTR|DDLOCK_WAIT, NULL );
		if( ddrval!=DD_OK )
		{
			if (ddrval != DDERR_SURFACELOST)
				debugf(NAME_Log, TEXT("D3D Driver: Lock on primary failed (%ls)"), *D3DError(ddrval));
			return;
		}
		...
		DWORD gamtbl[256];
		const UBOOL bDoGammaControl = (bGammaCorrectOutput && GammaControl);
		if(bDoGammaControl)
		{
	...
	void SetTexture( DWORD dwStage, FTextureInfo& Info, DWORD PolyFlags, UBOOL Precache )
	{
		...
		if (!ti)
		{
					...
					|| (VSize = Info.Mips[FirstMip]->VSize) > fi->PixelFormat->MaxHeight)
			{
				...
				{
					// elmuerte: ok, this is not really a fix but it works
					//appErrorf( TEXT("D3D Driver: Encountered oversize texture without sufficient mipmaps") );
					fi = (FTexFiller*)&FillerNPOT;
					Info.Mips[0]->USize = Info.USize = USize = fi->PixelFormat->MaxWidth;
					Info.Mips[0]->VSize = Info.VSize = VSize = fi->PixelFormat->MaxHeight;
					Info.Mips[0]->UBits = appCeilLogTwo(USize);
					Info.Mips[0]->VBits = appCeilLogTwo(VSize);
					FirstMip = 0;
					Info.NumMips = 1;
					debugf(TEXT("D3D: Encountered oversize texture without sufficient mipmaps. Uploading a white texture instead of %ls"), *Info.Texture->GetPathName());
					break;
				}
			}

			// stijn: Deal with NPOT textures without crashing
			if (Info.USize != 1 << Info.Mips[0]->UBits ||
				Info.VSize != 1 << Info.Mips[0]->VBits)
			{
				fi = (FTexFiller*)&FillerNPOT;
				Info.Mips[0]->USize = USize = 1 << Info.Mips[0]->UBits;
				Info.Mips[0]->VSize = VSize = 1 << Info.Mips[0]->VBits;
				debugf(TEXT("D3D: NPOT Textures are not supported. Uploading a white texture instead of %ls"), *Info.Texture->GetPathName());
			}
...
#if UNREAL_TOURNAMENT_OLDUNREAL
			ti->RealtimeChangeCount = Info.Texture ? Info.Texture->RealtimeChangeCount : 0;
#endif
			...
			/*if( ti->PixelFormat==&FormatP8 )
			{
				...
				if( FAILED(h=DirectDraw->CreatePalette(DDPCAPS_8BIT | DDPCAPS_ALLOW256 | DDPCAPS_INITIALIZE | (UseAlpha?DDPCAPS_ALPHA:0), pe, &ti->Palette, NULL)) )
					appErrorf(TEXT("CreatePalette failed (%ls)"),*D3DError(h));
			}
			*/
			unguard;
		}
		...
		{
			if (ti->RealtimeChangeCount != Info.Texture->RealtimeChangeCount)
				ti->RealtimeChangeCount = Info.Texture->RealtimeChangeCount;
			else
		...
		if( Info.bRealtimeChanged || (Info.Format==TEXF_RGBA7 && GET_COLOR_DWORD(*Info.MaxColor)==0xFFFFFFFF) )
		{
			...
			for( INT MipIndex=ti->FirstMip, ListIndex=0; ListIndex<Count; ListIndex++,MipIndex++ )
			{
				...
				if( FAILED(h=LockSurface->Lock(NULL,&ddsd,DDLOCK_NOSYSLOCK|DDLOCK_SURFACEMEMORYPTR|DDLOCK_WAIT|DDLOCK_WRITEONLY,NULL)) )
					appErrorf( TEXT("D3D Driver: Lock failed while loading system surface (%ls)"), *D3DError(h) );
	...
	static BOOL CALLBACK EnumDirectDrawsCallback( GUID* Guid, char* Description, char* Name, void* Context, HMONITOR Monitor )
	{
		...
		debugf(NAME_Init,TEXT("   %ls (%ls)"), Name?appFromAnsi(Name):TEXT("None"), Description?appFromAnsi(Description):TEXT("None") );
		return 1;
	}
	...
	UBOOL SetRes( INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )
	{
		...
		debugf(NAME_Init,TEXT("Unreal Direct3D support - internal revision 1.9b"));
		debugf(NAME_Init,TEXT("D3D Device: szDriver=%ls"),      appFromAnsi(DeviceIdentifier.szDriver));
		debugf(NAME_Init,TEXT("D3D Device: szDescription=%ls"), appFromAnsi(DeviceIdentifier.szDescription));
		...
		else
		{
			...
			BackSurfaceDesc.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_3DDEVICE | DDSCAPS_VIDEOMEMORY;
		...
		if( FAILED(h=Direct3D7->CreateDevice( IID_IDirect3DHALDevice, BackSurface7, &Direct3DDevice7)) ) // currently fails with DDERR_INVALIDOBJECT                     
		...
		guard(InitTextureFormats);
		{
			...
			for( TArray<DDPIXELFORMAT>::TIterator It(PixelFormats); It; ++It )
				if( It->dwFlags & DDPF_FOURCC )
				{
					...
					debugf(TEXT("D3D Driver: Supports FOURCC %ls"),Code);
				}
			unguard;
		}
		unguard;	
		...
		// VSync flip control.
		if( !(DirectDrawCaps.dwCaps2 & DDCAPS2_FLIPNOVSYNC) )
			debugf( NAME_Init, TEXT("D3D Driver: VSync flip disabling not available.") );
			...
			for( DWORD USize=DeviceDesc7.dwMinTextureWidth; USize<=Fmt->MaxWidth; USize*=2 )
			{
				for( DWORD VSize=DeviceDesc7.dwMinTextureHeight; VSize<=Fmt->MaxHeight; VSize*=2 )
				{
					...
					if( FAILED(h=DirectDraw7->CreateSurface(&SysSurfaceDesc, &S, NULL)) )
						appErrorf(TEXT("D3D Driver: CreateSurface for system failed: %ls"),*D3DError(h));
					do
						...
						if( Pool )
						{
							...
							for( INT c=0; c<Copies && Pool->TexCount<Pool->DesiredSwapCopies; c++ )
							{
								...
								{										
									...
									{
										if( Count==0 )
											appErrorf(TEXT("Failed to preallocate initial textures, %ix%i: %ls"),USize,VSize,*D3DError(h));																			
									}									
								}
							}
						}
	...
	UBOOL UnSetRes( const TCHAR* Msg, HRESULT h )
	{
		...
		if( Msg )
			debugf(NAME_Init,TEXT("%ls (%ls)"),Msg,*D3DError(h));
		Flush(1);
