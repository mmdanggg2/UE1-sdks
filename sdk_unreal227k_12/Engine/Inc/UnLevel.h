...
public:
	...
	ABrush* Brush()
	{
		...
		verify(Actors.Num()>=2);
		return CastChecked<ABrush>(Actors(1));
		unguardSlow;
	}
	INT GetActorIndex( AActor* Actor )
	{
		guard(ULevel::GetActorIndex);
		INT Result = Actors.FindItemIndex(Actor);
		verifyf((Result != INDEX_NONE), TEXT("Actor not found: %ls"), Actor->GetFullName());
		return Result;
		unguard;
	}
	ALevelInfo* GetLevelInfo()
	{
		...
		verify(Actors.Num() >= 1);
		return CastChecked<ALevelInfo>(Actors(0));
		unguardSlow;
	}
