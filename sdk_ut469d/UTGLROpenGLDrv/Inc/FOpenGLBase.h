...
/* Uniforms */ \
	GLEntry(PFNGLBINDBUFFERBASEPROC,glBindBufferBase)/* DUPLICATED IN GL3!! */\
	GLEntry(PFNGLBINDBUFFERRANGEPROC,glBindBufferRange)/* DUPLICATED IN GL3!! */ \
/* Vertex Array Object */ \
...
public:
	...
	static void CreatePersistentBuffer( GLuint& Buffer, GLenum Target, GLsizeiptr Size, const void* Data, GLbitfield PersistentFlags, GLenum MutableUsage);
...
inline void FOpenGLBase::CreatePersistentBuffer( GLuint& Buffer, GLenum Target, GLsizeiptr Size, const void* Data, GLbitfield PersistentFlags, GLenum MutableUsage)
{
	if ( SupportsPersistentMapping )
	{
		glGenBuffers(1, &Buffer);
		glBindBuffer(Target, Buffer);
		glBufferStorage(Target, Size, Data, PersistentFlags);
		glBindBuffer(Target, 0);
	}
	else
		CreateBuffer( Buffer, Target, Size, Data, MutableUsage);
}
