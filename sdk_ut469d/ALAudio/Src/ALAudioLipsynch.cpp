...
void UALAudioSubsystem::Lipsynch(ALAudioSoundInstance& Playing)
{
	...
	alGetBufferi(AudioHandle->ID, AL_SAMPLE_LENGTH_SOFT, &NumSamples);
	if ((Error = alGetError()) != AL_NO_ERROR)
    {
        warnf(TEXT("Failed to query number of samples (%s)."), appFromAnsi(alGetString(Error)));
        goto Cleanup;
    }
	alGetBufferi(AudioHandle->ID, AL_FREQUENCY, &SampleRate);
	if ((Error = alGetError()) != AL_NO_ERROR)
    {
	...
	guard(Download);
	alGetBufferSamplesSOFT(AudioHandle->ID, SampleIndex, NUM_LIPSYNCH_SAMPLES, AL_MONO_SOFT, AL_FLOAT_SOFT, SampleData);
	if ((Error = alGetError()) != AL_NO_ERROR)
    {
        warnf(TEXT("Failed to query sample data (%s)."), appFromAnsi(alGetString(Error)));
        goto Cleanup;
    }
	unguard;
	...
	// First output :-)
	//debugf( TEXT("Volume of speech samples for %s is (Min=%f,Avg=%f,Max=%f)."), *Pawn->FamiliarName, MinVolume, AvgVolume, MaxVolume );
	//debugf( TEXT("SampleRate is %i"), SampleRate );

	// Close the mouth if below a threshold.
	if (AvgVolume<0.07)
	{
		//debugf( TEXT("Shutting mouth for to low average volume (%f) on %s."), AvgVolume, *Pawn->FamiliarName );
	...
	{
		//debugf( TEXT("Shutting mouth for to low average FREQUENCY volume (%f) on %s."), AvgVolume, *Pawn->FamiliarName );
		Pawn->nextPhoneme = TEXT("X");
	}
	unguard;
...
Cleanup:
	...
	// Summary.
	//debugf( TEXT("Result for %s: %s"), *Pawn->FamiliarName, *Pawn->nextPhoneme );
	unguard;
}
#endif
