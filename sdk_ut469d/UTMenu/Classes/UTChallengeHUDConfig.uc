...
var UWindowCheckbox CrosshairTranslucentCheck;
var localized string CrosshairTranslucentText;
var localized string CrosshairTranslucentHelp;
...
function Created()
{
	...
	CrosshairTranslucentCheck = UWindowCheckbox(CreateControl(class'UWindowCheckbox', CenterPos, ControlOffset, CenterWidth, 1));
	CrosshairTranslucentCheck.SetText(CrosshairTranslucentText);
	CrosshairTranslucentCheck.SetHelpText(CrosshairTranslucentHelp);
	CrosshairTranslucentCheck.SetFont(F_Normal);
	CrosshairTranslucentCheck.Align = TA_Left;
	ControlOffset += 20;
	...
	DesiredHeight = ControlOffset + Min(Crosshair.VSize, 128) + 6;
...
{ 
	...
	CrosshairTranslucentCheck.bChecked = H.bTranslucentCrosshair;	
...
function LoadDefaultValues()
{
	...
	H.bTranslucentCrosshair = class'ChallengeHUD'.default.bTranslucentCrosshair;	
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	CrosshairTranslucentCheck.SetSize(CenterWidth, 1);
	CrosshairTranslucentCheck.WinLeft = CenterPos;	
...
{
	local int ControlWidth, ControlLeft, ControlRight;
	local int CenterWidth, CenterPos, CrosshairX, HUDX, CrosshairWidth, CrosshairHeight, Tmp;
	local ChallengeHUD H;
	...
	if (CrosshairScaleAutoCheck.bChecked)
	{
		if ( C.SizeX < 512 )
			...
			XScale = Clamp(int(0.1 + C.SizeX/640.0), 1, 2);
	}
	else
	{
    ...
    // stijn: adjust crosshair box dimensions for oversized crosshairs
    if (Crosshair.USize > 128 || Crosshair.VSize > 128)
	{
        Tmp = Max(Crosshair.USize, Crosshair.VSize);
		XScale = XScale * Tmp / 128;
		CrosshairWidth = Crosshair.USize * 128 / Tmp;
		CrosshairHeight = Crosshair.VSize * 128 / Tmp;
    }
	else
	{
        CrosshairWidth = Crosshair.USize;
		CrosshairHeight = Crosshair.VSize;
    }

	// stijn: divide XScale by GUI scale to ensure the crosshair matches its in-game dimensions
	XScale = XScale / UMenuRootWindow(Root).GUIScale;

	CrosshairX = (WinWidth - CrosshairWidth) / 2;
	T = GetLookAndFeelTexture();
	DrawUpBevel(C, CrosshairX - 3, CrosshairScaleSlider.WinTop + 20 - 3, CrosshairWidth + 6, CrosshairHeight + 6, T);
	bOldNoSmooth = C.bNoSmooth;
    C.bNoSmooth = !CrosshairSmoothCheck.bChecked;
	DrawStretchedTexture(C, CrosshairX, CrosshairScaleSlider.WinTop + 20, CrosshairWidth, CrosshairHeight, Texture'BlackTexture');
...
//	DrawClippedTexture(C, CrosshairX, CrosshairScaleSlider.WinTop + 20, CrossHair);
	OldStyle = C.Style;
	if (CrosshairTranslucentCheck.bChecked)
	    C.Style = GetPlayerOwner().ERenderStyle.STY_Translucent;
	else
		C.Style = GetPlayerOwner().ERenderStyle.STY_Normal;
	DrawStretchedTexture(C,
		CrosshairX + (1 - XScale) * CrosshairWidth / 2,
		CrosshairScaleSlider.WinTop + 20 + (1 - XScale) * CrosshairHeight / 2,
		CrosshairWidth * XScale,
		CrosshairHeight * XScale,
		CrossHair);
	C.Style = OldStyle;
...
function Notify(UWindowDialogControl C, byte E)
{
	...
	case DE_Change:
		...
		case CrosshairTranslucentCheck:
			CrosshairChanged();
			break;
		case HUDRSlider:
		case HUDGSlider:
...
function CrosshairChanged()
{
	...
	H.bTranslucentCrosshair = CrosshairTranslucentCheck.bChecked;	
...
defaultproperties
{
	...
	CrosshairTranslucentText="Translucent Crosshair"
	CrosshairTranslucentHelp="Enable to apply translucency to your crosshair."
