...
function Paint( Canvas C, float X, float Y )
{
	...
	ScrollingOffset = Clamp(ScrollingOffset, 0, Max(0, FadeLines - VisibleRows));
...
function Clear()
{
	...
	FadeFactor = default.FadeFactor;
}

function bool CheckMousePassThrough(float X, float Y)
{
	FadeFactor = default.FadeFactor*100;
...
{
	bNowAutoScrolling = False;
	ScrollingOffset = Clamp(ScrollingOffset - int(ScrollDelta), 0, Max(0, FadeLines - 1));
	return Super.MouseWheelUp(ScrollDelta);
}
