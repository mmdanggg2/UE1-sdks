...
class WBrowserMusic : public WBrowser
{
		...
		switch( Command )
		{
			...
			case IDMN_MB_DELETE:
			{
				...
				if( !GetPropResult.Len() )
					...
					appMsgf( TEXT("Can't delete music - Error: %s"), *GetPropResult );
			}
			break;
			...
			case IDMN_MB_RENAME:
			{
				...
				pComboPackage->SetCurrent( pComboPackage->FindStringExact( *Name) );
				RefreshGroups();
				RefreshMusicList();
			}
			break;
			...
			case IDMN_SB_EXPORT_MUS_ALL:
			{
				...
				for (TObjectIterator<UMusic> It; It; ++It)
				{
					...
					if (A->IsIn(Pck))
					{
						...
						Exported += GEditor->Exec(*FString::Printf(TEXT("OBJ EXPORT TYPE=MUSIC PACKAGE=\"%ls\" NAME=\"%ls\" FILE=\"%ls\""), *CurrentPackage, *Name, *OutName));
						Expected++;
					}
				}
				GFileManager->SetDefaultDirectory(appBaseDir());
	...
	void RefreshPackages(void)
	{
		...
		FStringOutputDevice GetPropResult = FStringOutputDevice();
		GEditor->Get(TEXT("OBJ"), TEXT("PACKAGES CLASS=MUSIC"), GetPropResult);
	...
	void RefreshGroups(void)
	{
		...
		FStringOutputDevice GetPropResult = FStringOutputDevice();
		GEditor->Get(TEXT("OBJ"), *FString::Printf(TEXT("GROUPS CLASS=MUSIC PACKAGE=\"%ls\""), *Package), GetPropResult);
	...
	inline void ListMusic(UMusic* M)
	{
		MusicList.AddItem(M);
	...
	void RefreshMusicList(void)
	{
		...
		FString Package = pComboPackage->GetString(pComboPackage->GetCurrent());
		FString Group = pComboGroup->GetString(pComboGroup->GetCurrent());
		FString Selected = pListMusic->GetString(pListMusic->GetCurrent());
		...
		else
		{
			for (TObjectIterator<UMusic> It; It; ++It)
			{
				if (Group != TEXT("") && !It->IsIn(Pck))
					continue;
				UPackage* P = FindOuter<UPackage>(*It);
				if (P == Pck && (Group == TEXT("") || Group == *FObjectName(P)))
					ListMusic(*It);
			}
		}

		INT Current = pListMusic->FindStringExact(*Selected);
		if (Current >= 0)
			pListMusic->SetCurrent(Current, 1);
