	...
	BOOL GetDataFromUser( void )
	{
		...
		else
		{
			...
			for (INT Pos = 0, N = Package.Len(); Pos < N; Pos++)
				...
				{
					appMsgf( TEXT("Invalid character in package name '%c'"), (*Package)[Pos]);
					return FALSE;
				}
			...
			for (INT Pos = 0, N = Name.Len(); Pos < N; Pos++)
				...
				{
					appMsgf( TEXT("Invalid character in class name '%c'"), (*Name)[Pos]);
					return FALSE;
				}
...
class WBrowserActor : public WBrowser
{
		...
		switch( Command )
		{
			case WM_TREEVIEW_RIGHT_CLICK:
			{
				...
				if (Target == pTreeView->hWnd)
				{
					...
					HMENU menu = GetSubMenu( AddHotkeys(LoadMenuW(hInstance, MAKEINTRESOURCEW(IDMENU_BrowserActor_Context))), 0 );
					TrackPopupMenu( menu,
				...
				else if (Target == pPackagesList->hWnd)
				{
					...
					if (Selected >= 0)
					{
						HMENU menu = GetSubMenu( AddHotkeys(LoadMenuW(hInstance, MAKEINTRESOURCEW(IDMENU_BrowserActor_Package))), 0 );
						TrackPopupMenu( menu,
							TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RIGHTBUTTON,
							ptScreen.x, ptScreen.y, 0,
							hWnd, NULL);
						...
						Files) && Files.Num() > 0)
					{
						...
						for (TObjectIterator<UClass> It; It; ++It)
							if ((It->ClassFlags & CLASS_EditorExpanded))
								Previous.AddItem(It->GetFullNameSafe());
						...
						int i;
						for (TObjectIterator<UClass> It; It; ++It)
							if ((It->ClassFlags & CLASS_EditorExpanded) && !Previous.FindItem(It->GetFullNameSafe(), i))
								It->ClassFlags &= ~CLASS_EditorExpanded;
				...
				if( l_dlg.DoModal( GEditor->CurrentClass ? *FObjectPathName(GEditor->CurrentClass) : TEXT("Engine.Actor") ) )
				{
					...
					if (GEditor->CurrentClass)
					{
						...
						FString S = FString::Printf(
							TEXT("//=============================================================================%c%c// %ls.%c%c//=============================================================================%c%cclass %ls expands %ls;%c%c"),
							ch13, ch10,
							*l_dlg.Name, ch13, ch10,
							ch13, ch10,
							*l_dlg.Name, *l_dlg.ParentClass.Mid(l_dlg.ParentClass.InStr(TEXT("."))+1), ch13, ch10);
			...
			case ID_FileOpen:
			{
				...
				{
					for( int x = 0 ; x < Files.Num() ; x++ )
						GEditor->Exec(*FString::Printf(TEXT("CLASS LOAD FILE=\"%ls\""), *(Files(x))));
		...
		if (GetSaveNameWithDialog(
			...
			*FString::Printf(TEXT("Save %ls Package"), *Package),
			...
			GEditor->Exec(*FString::Printf(
				TEXT("OBJ SAVEPACKAGE PACKAGE=\"%ls\" FILE=\"%ls\""), *Package, *File));
	...
	void ExportScripts(EExportType Type)
	{
		...
		for (TObjectIterator<UClass> It; It; ++It)
		{
			...
			Path += FString::Printf(PATH_SEPARATOR TEXT("%ls.uc"), It->GetName());
	...
	void ReopenClass(UClass* Class, HTREEITEM hti, UClass* FirstVisible, HTREEITEM* htiFirstVisible)
	{
		...
		for (TObjectIterator<UClass> It; It; ++It)
		{
			...
			{
				UBOOL bChildren = 0;
				HTREEITEM htiChild = pTreeView->AddItem(*GetClassDisplayName(C, bChildren), hti, bChildren);
	...
	void AddChildren(const TCHAR* pParentName, HTREEITEM hti, UBOOL Expand)
	{
		...
		for (TObjectIterator<UClass> It; It; ++It)
		{
			...
			{
				UBOOL bChildren = 0;
				pTreeView->AddItem(*GetClassDisplayName(C, bChildren), hti, bChildren);
			}
		}
		unguard;
	}
	...
	void OnPackagesListDblClk( void )
	{
		...
		UClass* Last = NULL;
		for (TObjectIterator<UClass> It; It; ++It)
			if (FObjectName(It->GetOuterUPackage()) == Current)
			{
				Count++;
				Last = *It;
			}
		if (Count == 0 || !GWarn->YesNof(TEXT("Open all (%i) classes for package %ls?"), Count, *Current))
			return;
		for (TObjectIterator<UClass> It; It; ++It)
			if (FObjectName(It->GetOuterUPackage()) == Current)
				GCodeFrame->AddClass(*It, -1, -1, *It != Last);
