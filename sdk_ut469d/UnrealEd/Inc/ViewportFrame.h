...
class WVFToolBar : public WWindow, public WViewportWindowContainer
{
	...
	void OnRightButtonUp()
	{
		...
		HMENU l_menu = GetSubMenu( AddHotkeys(LoadMenuW(hInstance, MAKEINTRESOURCEW(IDMENU_VF_CONTEXT))), 0 );
...
class WViewportFrame : public WWindow, public WViewportWindowContainer
{
	...
	{
		int RendMap, ShowBackdrop, ShowRealTimeBackdrop, ShowCoordinates, ShowPaths, ShowDistanceFog, ShowPathsPreview, ShowLightColorIcon, ShowActors;
		...
		DWORD ShowFlags = SHOW_Menu | SHOW_Frame | ShowActors | SHOW_Brush | SHOW_StandardView | SHOW_ChildWindow | SHOW_MovingBrushes;
