...
function KeyDown(int Key, float X, float Y)
{
	...
	case P.EInputKey.IK_Up:
		if (SelectedItem != None)
			for (CurItem = SelectedItem.Prev; CurItem != None; CurItem = CurItem.Prev)
				if (CurItem.ShowThisItem())
				{
					SetSelectedItem(UWindowListBoxItem(CurItem));
					break;
				}
		break;
	case P.EInputKey.IK_Down:
		if (SelectedItem != None)
			for (CurItem = SelectedItem.Next; CurItem != None; CurItem = CurItem.Next)
				if (CurItem.ShowThisItem())
				{
					SetSelectedItem(UWindowListBoxItem(CurItem));
					break;
