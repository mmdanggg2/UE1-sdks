...
#define unguardTranscode  unguardf(( TEXT("(InputFormat=%s,InputBytes=%i,OutputFormat=%s,OutputBytes=%i,Width=%i,Height=%i,Depth=%i"), *FTextureFormatString(InputFormat), InputData.Num(), *FTextureFormatString(OutputFormat), OutputData.Num(), Width, Height, Depth ));
...
public:
	...
	virtual UBOOL Transcode(const BYTE InputFormat, TArray<BYTE> InputData, const BYTE OutputFormat, TArray<BYTE>& OutputData, const INT Width, const INT Height, const INT Depth = 1, FOutputDevice* Error = GError)
	{
		...
		{
			Error->Logf(TEXT("InputFormat and OutputFormat are both %s."), *FTextureFormatString(InputFormat));
			return 0;
		}
		if (FIsMetaFormat(InputFormat))
		{
			Error->Logf(TEXT("InputFormat is MetaFormat %s."), *FTextureFormatString(InputFormat));
			return 0;
		}
		if (FIsMetaFormat(OutputFormat))
		{
			Error->Logf(TEXT("OutputFormat is MetaFormat %s."), *FTextureFormatString(OutputFormat));
			return 0;
		}
		if (!SupportsTranscoding(InputFormat, OutputFormat))
		{
			Error->Logf(TEXT("Transcoding from %s to %s is not supported."), *FTextureFormatString(InputFormat), *FTextureFormatString(OutputFormat));
			return 0;
		}
		if (Width < 1)
		{
