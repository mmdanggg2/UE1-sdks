...
function string GetInfo()
{
	...
	if ( WorldLog != None )
		ResultSet $= "\\wantworldlog\\true";
	else
		ResultSet $= "\\wantworldlog\\false";

	return ResultSet;
}

// OldUnreal: moved into C++ because this can cause heavy lag spikes on the server if we don't do it efficiently
// See OldUnreal/UnrealTournamentPatches/issues/1218
native function string GetMutatorList();
...
function string GetRules()
{
	...
	if( EnabledMutators == "" )
	{
        EnabledMutators = GetMutatorList();
	    if(EnabledMutators == "")
	        EnabledMutators = " ";		
    }	
	...
	if(EnabledMutators != " ")
		ResultSet = "\\mutators\\"$EnabledMutators;
		
	// utpg: mutator/browser fix end
	ResultSet $= "\\listenserver\\"$string(Level.NetMode==NM_ListenServer)
	...
	if (Other.bIsPlayer)
	{
		...
		if ( (DamageType == 'SpecialDamage') && (SpecialDamageString != "") )
		{
			if (Killer != None && Killer.PlayerReplicationInfo != None)
				Message = Killer.PlayerReplicationInfo.PlayerName;
			if (Other != None && Other.PlayerReplicationInfo != None)
				OtherWeapon = Other.PlayerReplicationInfo.PlayerName;
