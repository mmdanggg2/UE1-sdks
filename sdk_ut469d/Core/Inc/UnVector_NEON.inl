...
{
	return FVector_vst(vld(V) * Scale);
}
...
inline FVector FVector::operator+( const FVector& V ) const
{
	return FVector_vst(vld(*this) + vld(V));
}
...
{
	return FVector_vst(vld(*this) * Scale);
}
...
inline UBOOL FVector::operator==( const FVector& V ) const
{
	...
	return (vget_lane_u64(vreinterpret_u64_u16(cmp16), 0) & 0x0000FFFFFFFFFFFFull) == 0x0000FFFFFFFFFFFFull;
}
...
{
	return FVector_vst(vnegq_f32(vld(*this)));
}
...
{
	*this = FVector_vst(vld(V));
	return *this;
}

inline FVector FVector::operator+=( const FVector& V )
{
	*this = FVector_vst(vld(*this) + vld(V));
	return *this;
}
...
{
	*this = FVector_vst(vld(*this) * Scale);
	return *this;
}

inline FVector FVector::operator/=( FLOAT V )
{
	*this = FVector_vst(vld(*this) / V);
	return *this;
}
