...
function ScoreKill(pawn Killer, pawn Other)
{
	...
	if ( GoalTeamScore > 0 && Killer != None
	   && Killer.bIsPlayer
	...
	else if ( bOverTime && Killer != None && Killer.bIsPlayer )
	{
       EndGame("timelimit");
	}
}
...
function string GetRules()
{
	...
	ResultSet $= "\\goalteamscore\\"$int(GoalTeamScore);
	Resultset $= "\\minplayers\\"$MinPlayers;
	Resultset $= "\\changelevels\\"$bChangeLevels;
	ResultSet $= "\\maxteams\\"$MaxTeams;
	ResultSet $= "\\balanceteams\\"$bBalanceTeams;
	ResultSet $= "\\playersbalanceteams\\"$bPlayersBalanceTeams;
	ResultSet $= "\\friendlyfire\\"$int(FriendlyFireScale*100)$"%";
