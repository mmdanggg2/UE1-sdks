...
// stijn: if true, you will cap instantly if you pick up the enemy flag while it is within the
// collision radius of your home flag base.
// In v436, you would have to walk out of the collision radius first and then walk back in.
var config bool bFixFlagBasePickup;
...
function ScoreKill(pawn Killer, pawn Other)
{
	...
	else if ( killer != None )
	{
		...
		if ( Killer.bIsPlayer && Other.bIsPlayer && 
			Killer.PlayerReplicationInfo != None && Other.PlayerReplicationInfo != None &&
			(Killer.PlayerReplicationInfo.Team != Other.PlayerReplicationInfo.Team) )
			Killer.PlayerReplicationInfo.Score += 1;
	}
	if ( bAltScoring && (Killer != Other) && (killer != None) && Other.bIsPlayer && Other.PlayerReplicationInfo != None )
		Other.PlayerReplicationInfo.Score -= 1;
...
function Actor SetDefenseFor(Bot aBot)
{
	if (aBot.PlayerReplicationInfo == None || 
		aBot.PlayerReplicationInfo.Team >= ArrayCount(CTFReplicationInfo(GameReplicationInfo).FlagList))
		return None;
...
function bool FindSpecialAttractionFor(Bot aBot)
{
	...
	if ( (aBot.Enemy == None) && (aBot.Orders != 'Defend') )
	{
		...
		if ( EnemyFlag.Holder != None && ((Dist > 500) || (VSize(EnemyFlag.Holder.Velocity) > 230)
...
defaultproperties
{
	 ...
	 GoalTeamScore=3
	 bAutoRespawnFlags=false
	 bFixFlagBasePickup=true
}
