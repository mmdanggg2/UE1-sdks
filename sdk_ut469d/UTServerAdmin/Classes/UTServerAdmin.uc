...
function String GenerateMutatorListSelect(ListItem MutatorList)
{
	...
	for (TempItem = MutatorList; TempItem != None; TempItem = TempItem.Next) {
		...
		ResponseStr $= "<option value=\""$TempItem.Data$"\""$SelectedStr$">"$TempItem.Tag$"</option>";
	}
	return ResponseStr;
}
...
function UpdateDefaultMaps(String GameType, ListItem TempItem)
{
	...
	GameClass.Default.MapListType.Static.StaticSaveConfig();
}
...
function String GenerateGameTypeOptions(String CurrentGameType)
{
	...
	for (TempItem = GameTypeList; TempItem != None; TempItem = TempItem.Next)
	{
		...
		OptionStr $= "<option value=\""$TempItem.Data$"\""$SelectedStr$">"$TempItem.Tag$"</option>";
	}
	return OptionStr;
}
...
function String GenerateMapListOptions(String GameType, String MapListType)
{
	...
	while( (NextDefault != "") && (NumDefaultClasses < 50) )
	{
		...
		OptionStr $= "<option value=\""$NextDefault$"\""$SelectedStr$">"$NextDesc$"</option>";
...
event Query(WebRequest Request, WebResponse Response)
{
	...
	case DefaultsRestartPage:
		Response.Subst("Area", "Please Wait");
		Response.Subst("Section", "");
...
function QueryCurrentPlayers(WebRequest Request, WebResponse Response)
{
	...
	{
		Minutes = Max((Level.Game.GameReplicationInfo.RemainingTime / 60),0);
		Seconds = Level.Game.GameReplicationInfo.RemainingTime % 60;
		S = Level.Game.GameReplicationInfo.GameName@"in"@Level.Title@"("@Minutes$":"$PadLeft(Left(string(Seconds),2),2,"0")@"remaining )";
	}
	else S = Level.Game.GameReplicationInfo.GameName@"in"@Level.Title;
			...
			if(Request.GetVariable("BanPlayer"$string(P.PlayerReplicationInfo.PlayerID)) != "")
			{
				...
				if(Level.Game.CheckIPPolicy(IP))
				{
					...
					if (Level.Game.bLogAdminActions)
						log("Webadmin banned"@P.PlayerReplicationInfo.PlayerName@PlayerPawn(P).GetPlayerNetworkAddress()@"during"@Left(string(Level), InStr(string(Level), "."))$".unr",'AdminAction');
				}
				P.Destroy();
			}
			else
	...
	for (P=Level.PawnList; P!=None; P=P.NextPawn) {
		if (P.bIsPlayer && !P.bDeleteMe && UTServerAdminSpectator(P) == None && P.PlayerReplicationInfo != None) {
			PawnCount++;
			TempItem = new(None) class'ListItem';
...
{
	local ListItem TempItem;
	local int Count, i, Minutes, Seconds;
	local string S;
...
function QueryDefaultsServer(WebRequest Request, WebResponse Response)
{
	...
	local string LoginSeconds, MaxAttempts;
	local bool bDoUplink, bWorldLog, bAdminLogging;
	local int LoginAction;
	...
	LoginSeconds = string(Level.Game.default.LoginDelaySeconds);
	MaxAttempts = string(Level.Game.default.MaxLoginAttempts);
	...
	LoginAction = int(Level.Game.ActionToTake);
	bAdminLogging = Level.Game.default.bLogAdminActions;
	...
	if (Request.GetVariable("Apply", "") != "")
	{
		...
		LoginSeconds = Request.GetVariable("LoginDelay", "");
		MaxAttempts = Request.GetVariable("MaximumLogins", "");
		bAdminLogging = bool(Request.GetVariable("LogAdmins", ""));
		...
		LoginAction = int(Request.GetVariable("LoginReaction", string(LoginAction)));
		...
		Level.Game.default.LoginDelaySeconds = float(LoginSeconds);
		Level.Game.default.MaxLoginAttempts = int(MaxAttempts);
		Level.Game.default.bLogAdminActions = bAdminLogging;
		if (Level.Game.default.ActionToTake != LoginAction)
		{
			switch (LoginAction)
			{
				case 0: Level.Game.default.ActionToTake = DO_Nothing; break;
				case 1: Level.Game.default.ActiontoTake = DO_Log; break;
				case 2: Level.Game.default.ActionToTake = DO_DisableLogin; break;
				case 3: Level.Game.default.ActionToTake = DO_KickPlayer; break;
				case 4: Level.Game.default.ActionToTake = DO_KickBanPlayer; break;
			}
		}
	...
	Response.Subst("LoginDelay", Left(LoginSeconds,4));
	Response.Subst("MaximumLogins", MaxAttempts);
	Response.Subst("Reaction"$LoginAction, " selected");

	if (bAdminLogging)
		Response.Subst("LogAdmins", " checked");
	if (bDoUplink)
...
function QueryDefaultsIPPolicy(WebRequest Request, WebResponse Response)
{
	...
	if(Request.GetVariable("Update") != "")
	{
		...
		if(i == -1)
			for(i = 0; i<ArrayCount(Level.Game.IPPolicies) && Level.Game.IPPolicies[i] != ""; i++);
		if(i < ArrayCount(Level.Game.IPPolicies))
	...
	if(Request.GetVariable("Delete") != "")
	{
		...
		if(i > 0)
		{
			for(i = i; (i<ArrayCount(Level.Game.IPPolicies) - 1) && Level.Game.IPPolicies[i] != ""; i++)
				Level.Game.IPPolicies[i] = Level.Game.IPPolicies[i + 1];

			if(i == (ArrayCount(Level.Game.IPPolicies) - 1))
...
function QueryDefaultsMD5Ignore(WebRequest Request, WebResponse Response)
{
  ...
  {
    tmp2 = Left(tmp, i);
    md5list $= Mid(tmp2, 1, Len(tmp2)-2)$chr(10);
    tmp = Mid(tmp, i+1);
    i = InStr(tmp, ",");
  }
  md5list $= Mid(tmp, 1, Len(tmp)-2);
