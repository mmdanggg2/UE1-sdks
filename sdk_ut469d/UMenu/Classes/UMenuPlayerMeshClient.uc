...
function Tick(float Delta)
{
	...
	if (LeftButton.bMouseDown) {
		ViewRotator.Yaw += 512*EtalonFPS*Delta;
	} else if (RightButton.bMouseDown) {
		ViewRotator.Yaw -= 512*EtalonFPS*Delta;
	}
	if (bRotate)
		ViewRotator.Yaw += 128*EtalonFPS*Delta;
}
...
function SetMesh(mesh NewMesh)
{
	...
	if(MeshActor.Mesh != None && MeshActor.HasAnim('Breath3'))
...
{
	if ( !MyMesh.HasAnim('Breath3') || MyMesh.AnimSequence == 'Breath3' )
	{
		if (MyMesh.HasAnim('All'))
