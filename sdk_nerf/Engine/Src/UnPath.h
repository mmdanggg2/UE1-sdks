...
#define MAXCOMMONRADIUS 50 //max radius to consider in building paths
#define MAXCOMMONHEIGHT 60
#define MINCOMMONHEIGHT 48 //min typical height for non-human intelligent creatures
#define MINCOMMONRADIUS 35 //min typical radius for non-human intelligent creatures
#define COMMONRADIUS    40 //max typical radius of intelligent creatures
#define HUMANRADIUS     35 //normal player pawn radius
#define HUMANHEIGHT     52 //normal playerpawn height
...
public:
	...
	void initialize(const FVector &spot, const FVector &dir, int mrk, const int beac, int left)
	{
		Location = spot;
		Direction = dir;
		marked = mrk;
...
private:
	...
	void exploreWall(FVector moveDirection);
	...
	int boundedReachable(FVector start,FVector destination);
