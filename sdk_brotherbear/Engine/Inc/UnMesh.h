...
class ENGINE_API UMesh : public UPrimitive
{
	...
	virtual void GetTriVerts(int, int* const) const;
...
//			return Owner->GetSkin( Count );
	...
	virtual FVector GetRootMovement(AActor*);
	virtual void AdjustRootMovement(AActor*, const FVector&);
	virtual FVector AnimCycleMovement(AActor*);
	virtual FCoords GetBoneCoords(AActor*, INT) const;
