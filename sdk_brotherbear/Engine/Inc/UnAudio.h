...
class ENGINE_API UAudioSubsystem : public USubsystem
{
	...
	virtual void UnregisterSound( USound* Sound, INT )=0;
	virtual UBOOL PlaySound( AActor* Actor, INT Id, USound* Sound, FVector Location, FLOAT Volume, FLOAT Radius, FLOAT Pitch, INT, FLOAT, FLOAT)=0;
	virtual INT StopSound( AActor*, INT, USound*, FLOAT)=0;
	virtual INT StopSound(AActor*, USound*) = 0;
	virtual INT PlayMusic(FString, FLOAT) = 0;
	virtual INT StopMusic(INT, FLOAT) = 0;
	virtual INT StopAllMusic(FLOAT) = 0;
	virtual void NoteDestroy( AActor* Actor )=0;
	virtual UViewport* GetViewport()=0;
	virtual void CleanUp() {};
	virtual void PostRender( FSceneNode* Frame )=0;
	virtual INT ModifySound(AActor*, INT, USound*, unsigned char, FLOAT) = 0;
