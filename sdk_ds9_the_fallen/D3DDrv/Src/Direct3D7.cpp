...
//#define MAX_TEX_SIZE 512 // 512 works but may slow down framerate because of texture memory thrashing.
#define MAX_TEX_SIZE 2048 // 512 works but may slow down framerate because of texture memory thrashing.
//#define MAX_TEX_SIZE 4096 // 512 works but may slow down framerate because of texture memory thrashing. CRASHS
...
class DLL_EXPORT UD3DRenderDevice : public URenderDevice
{
  DECLARE_CLASS(UD3DRenderDevice,URenderDevice,CLASS_Config)
	...
	// NERF Out of OpenGLDrv
	static QSORT_RETURN CDECL CompareRes( const FVector* A, const FVector* B )
	{
	...
	UBOOL Exec( const TCHAR* Cmd, FOutputDevice& Ar )
	{
		...
		if( DirectDraw7 && ParseCommand(&Cmd,TEXT("GetRes")) )
		{
			TArray<FVector> Res;
			...
			// NERF Out of OpenGLDrv
			appQsort( &Res(0), Res.Num(), sizeof(FVector), (QSORT_COMPARE)CompareRes );

			// NERF: these seems to be no script limitation
