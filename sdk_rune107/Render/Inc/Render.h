...
public:
	virtual void Init()=0;
	virtual void Exit()=0;
	virtual DWORD SetupForActor( FSceneNode* Frame, AActor* Actor, struct FVolActorLink* LeafLights, FActorLink* Volumetrics, FVector)=0;
	...
	virtual FPlane PointLight(FTransSample& Point, DWORD ExtraFlags)=0;
...
class RENDER_API URender : public URenderBase
{
	...
	void DrawActorSprite( FSceneNode* Frame, FDynamicSprite* Sprite, const FCoords& parentCoords);
	...
	void DrawSkeletalMesh(FSceneNode* frame, AActor* actor, FSpanBuffer*, AZoneInfo*, const FCoords& coords, FVolActorLink*, FActorLink*, DWORD, const FCoords& parentCoords);
	void DrawParticleSystem(FSceneNode* coords, AActor* actor, FSpanBuffer* spanBuffer, const FCoords& parentCoord);
