...
void regSet( HKEY Base, FString Dir, FString Key, FString Value )
{
	guard(regSet);
	HKEY hKey = NULL;
	LSTATUS Status = RegCreateKeyW( Base, *Dir, &hKey);
	if (Status != ERROR_SUCCESS)
	{
		if (Status == ERROR_ACCESS_DENIED || Status == ERROR_ELEVATION_REQUIRED)
			appMsgf(TEXT("Please run installer from the Administrator account."));
		// call again for get proper error message
		verifyError(RegCreateKeyW( Base, *Dir, &hKey) == ERROR_SUCCESS);
	}
	verifyError(RegSetValueExW( hKey, *Key, 0, REG_SZ, (BYTE*)*Value, (Value.Len()+1)*sizeof(TCHAR) )==ERROR_SUCCESS);
	verifyError(RegCloseKey( hKey )==ERROR_SUCCESS);
	unguard;
}
...
static UBOOL IsDrive( const TCHAR* Path )
{
	if( appStricmp(Path,TEXT(""))==0 )
		return 1;
	INT PathLen = appStrlen(Path);
	if( PathLen == 2 && appToUpper(Path[0])!=appToLower(Path[0]) && Path[1]==':' && Path[2]==0 )
		return 1;
	else if( appStricmp(Path,TEXT("\\"))==0 )
		return 1;
	else if( appStricmp(Path,TEXT("\\\\"))==0 )
		return 1;
	else if( PathLen >= 2 && Path[0]=='\\' && Path[1]=='\\' && !appStrchr(Path+2,'\\') )
		return 1;
	else if( PathLen >= 2 && Path[0]=='\\' && Path[1]=='\\' && appStrchr(Path+2,'\\') && !appStrchr(appStrchr(Path+2,'\\')+1,'\\') )
		return 1;
	else
		return 0;
}
...
public:
	...
	{
		guard(FComponentItem::ToggleSelection);
	...
	{
		guard(FComponentItem::OnItemDoubleClick);
		ToggleSelection();
		unguard;
	}
...
BOOL IsAlreadyRunning(const TCHAR* Path = NULL)
{
	if (Path == NULL)
		Path = appBaseDir();
	else if (FString(Path) == appBaseDir())
		return FALSE;
	INT Count=0;
	RetryMutex:
	HANDLE hMutex = CreateMutexW( NULL, 0, RUNNING_MUTEX(Path) );
...
class WFilerPageFolder : public WWizardPage, public FControlSnoop
{
	...
	WWizardPage* GetNext()
	{
		...
		&&	(*NewFolder)[2]==PATH_SEPARATOR[0] )
		{
			...
			{
				if (IsAlreadyRunning(*(NewFolder + PATH_SEPARATOR + TEXT("System") + PATH_SEPARATOR)))
					return NULL;
	...
	void OnReset()
	{
		...
		FString BaseDir;
		if (GFileManager->FileSize(TEXT("Detected.ini"))<0)
			BaseDir = Manager->RegistryFolder;
		else 
		{
			BaseDir = appBaseDir();
			if (BaseDir.Right(1) == PATH_SEPARATOR)
				BaseDir = BaseDir.LeftChop(1);
			BaseDir = BaseDir.Left(BaseDir.InStr(PATH_SEPARATOR, 1));
		}

		Folder.SetText( *BaseDir );
		Update();
		unguard;
	}
	void OnBrowse()
...
class WFilerPageUninstall : public WWizardPage
{
	...
	WWizardPage* GetNext()
	{
		guard(WFilerPageUninstall::GetNext);
...
class WFilerPageWelcome : public WWizardPage
{
	...
	void OnInitDialog()
	{
		...
		{
			Name[0] = '\0';
			INT LangId = 0, SubLangId = 0;
			TArray<FString> Paths;
			FString Path;
			if (!FLocalizeFileCache::FindLocaleFiles(Paths, TEXT("Core"), *Results(i).Object))
				Path = US + TEXT("Core.") + Results(i).Object;
			else
				Path = Paths(0);
			...
			if( appStricmp(*Results(i).Object,TEXT("int"))==0 )
				Current = i;
			if (!appStrlen(Name)) // fallback on fail get description
				appStrncpy(Name, *Results(i).Object, Current == i ? 4 : 3); // first two letters for all except int.
...
class WFilerPageUninstallComponents : public WFilerPageComponentsBase
{
	...
	void OnGroupChange( class FComponentItem* Group )
	{
		guard(WFilerPageUninstallComponents::OnGroupChange);
		INT i, Added;
	...
	const TCHAR* GetNextText()
	{
		guard(WFilerPageUninstallComponents::GetNextText);
		return Dependencies.Num() ? WWizardPage::GetNextText() : NULL;
		unguard;
	}
	WWizardPage* GetNext()
	{
		guard(WFilerPageUninstallComponents::GetNext);
		return Dependencies.Num() ? new WFilerPageUninstall(Owner) : NULL;
		unguard;
	}
	void OnInitDialog()
	{
		guard(WFilerPageUninstallComponents::OnInitDialog);
		WWizardPage::OnInitDialog();
		Components.OpenChildWindow( IDC_ComponentsHolder );
	...
	UBOOL GetShow()
	{
		guard(WFilerPageUninstallComponents::GetShow);
...
class WFilerPageAutoPlay : public WWizardPage
{
	...
	{
		return ShowInstallOptions ? NULL : Manager->MustReboot ? LocalizeGeneral(TEXT("RebootButton")) : LocalizeGeneral(TEXT("FinishButton"), TEXT("Window"));
	}
	const TCHAR* GetNextText()
	{
		return (Manager->MustReboot && !ShowInstallOptions) ? LocalizeGeneral(TEXT("ExitButton")) : NULL;
	}
...
INT WINAPI WinMain( HINSTANCE hInInstance, HINSTANCE hPrevInstance, char* InCmdLine, INT nCmdShow )
{
	...
	{		
		...
		appInit( GPackage, *appPlatformBuildCmdLine(1), nullptr, &Log, &Error, &Warn, &FileManager, FConfigCacheIni::Factory, 0 );
