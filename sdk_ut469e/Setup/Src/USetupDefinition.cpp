...
void USetupDefinition::InstallTree( const TCHAR* Action, FInstallPoll* Poll, void (USetupDefinition::*Process)( FString Key, FString Value, UBOOL Selected, FInstallPoll* Poll ), USetupGroup* SetupGroup, UBOOL Selected )
{
	...
	unguardf(( TEXT("(%s %s)"), Action, *FObjectFullName(SetupGroup) ));
}
...
void USetupDefinition::DoUninstallSteps( FInstallPoll* Poll )
{
	...
	if( Refs==0 )
	{
		...
		RemoveEmptyDirectory( *appFilePathName(*SetupIniFile) );
	}
...
void USetupDefinition::ProcessCopy( FString Key, FString Value, UBOOL Selected, FInstallPoll* Poll )
{
	...
	if( Selected && Key==TEXT("File") )
	{
		...
		if( Info.Lang==TEXT("") || Info.Lang==UObject::GetLanguage() )
		{
			...
			if( !GFileManager->MakeDirectory( *appFilePathName(*FullDest), 1 ) )
...
void USetupDefinition::CheckIniFile(FString IniFile)
{
	guard(USetupDefinition::CheckIniFile);
	if (GFileManager->FileSize(*IniFile) > 0)
		return;
	FString BaseName = appFileBaseName(*IniFile);
	if (!appStricmp(*BaseName, TEXT("User.ini")))
	{
		FString Default = appFilePathName(*IniFile) * TEXT("DefUser.ini");
		// Create User.ini from DefUser.ini.
		FString S;
		if (!appLoadFileToString( S, *Default, GFileManager))
			appErrorf(LocalizeError("MisingIni"), *Default);
		appSaveStringToFile(S, *IniFile);
	}
	else if (!appStricmp(*BaseName, *(Product + TEXT(".ini"))))
	{
		FString Default = appFilePathName(*IniFile) * TEXT("Default.ini");
...
void USetupDefinition::ProcessUninstallRemove( FString Key, FString Value, FInstallPoll* Poll )
{
	...
	if( (Key==TEXT("File") || Key==TEXT("Delete")) && UpdateRefCount(*Key,*Value,-1)==0 )
	{
		...
		RemoveEmptyDirectory( *appFilePathName(*(DestPath * Value)) );
	}
