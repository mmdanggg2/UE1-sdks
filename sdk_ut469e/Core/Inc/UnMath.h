...
//
class alignas(VECTOR_ALIGNMENT) CORE_API_EXPORT FVector 
{
public:
	// Variables.
	FLOAT X,Y,Z;
	...
	CORE_API_IMPORT FVector operator^( const FVector& V ) const;
	...
	UBOOL IsZero() const;
	UBOOL IsNan() const;
	CORE_API_IMPORT UBOOL Normalize();
	CORE_API_IMPORT UBOOL NormalizeSlow();
	...
	CORE_API_IMPORT UBOOL Booleanize();

	// Transformation.
	CORE_API_IMPORT FVector TransformVectorBy( const FCoords& Coords ) const;
	CORE_API_IMPORT FVector TransformPointBy( const FCoords& Coords ) const;
	FVector MirrorByVector( const FVector& MirrorNormal ) const;
	FVector MirrorByPlane( const FPlane& MirrorPlane ) const;
	FVector PivotTransform(const FCoords& Coords) const;

	// Complicated functions.
	CORE_API_IMPORT FRotator Rotation();
	CORE_API_IMPORT void FindBestAxisVectors( FVector& Axis1, FVector& Axis2 );
	CORE_API_IMPORT FVector SafeNormal() const;
	CORE_API_IMPORT FVector SafeNormalSlow() const;
	...
	CORE_API_IMPORT FString String() const
	{
	...
	CORE_API_IMPORT FString StringRaw() const
	{
		guardSlow(FVector::String);
...
#pragma pack(push, 4)
class CORE_API FPackedVector 
{
public:
	FLOAT X, Y, Z;

	FPackedVector() = default;

	FPackedVector(const FVector& V)
	{
		X = V.X;
		Y = V.Y;
		Z = V.Z;
	...
	FPackedVector& operator=(const FVector&& V)
	{
		X = V.X;
		Y = V.Y;
		Z = V.Z;
	...
	// Serializer.
	friend FArchive& operator<<(FArchive& Ar, FPackedVector& V)
	{
		return Ar << V.X << V.Y << V.Z;
	}
...
class alignas(VECTOR_ALIGNMENT) CORE_API_EXPORT FPlane
{
public:
	// Variables.
	FLOAT X,Y,Z,W;
	...
	CORE_API_IMPORT FPlane( FVector InNormal, FLOAT InW );
	CORE_API_IMPORT FPlane( FVector InBase, const FVector &InNormal );
	CORE_API_IMPORT FPlane( FVector A, FVector B, FVector C );
	...
	// Functions.
	CORE_API_IMPORT FLOAT PlaneDot( const FVector &P ) const;
	FPlane Flip() const;
	CORE_API_IMPORT FPlane TransformPlaneByOrtho( const FCoords &Coords ) const;
...
public:
	...
	CORE_API_IMPORT FBox( const FVector* Points, INT Count );
	...
	CORE_API_IMPORT FBox TransformBy( const FCoords& Coords ) const;
...
public:
	...
	FSphere( FBox* Box, const FVector* Pts, INT Count );
...
#define THRESH_SPLIT_POLY_PRECISELY		(0.05f)		/* A plane exactly splits a polygon */
...
inline FCoords& FCoords::operator*=( const FScale &Scale )
{
	...
	// Apply scaling.
	if (Scale.Scale.X != 1.0f || Scale.Scale.Y != 1.0f || Scale.Scale.Z != 1.0f)
	{
		XAxis    *= Scale.Scale;
		YAxis    *= Scale.Scale;
		ZAxis    *= Scale.Scale;
