...
class CORE_API ULinkerLoad : public ULinker, public FArchive
{
	...
	// Net downgrade status
	UBOOL					LoadingExports;
	INT						LastExportNameIndex;
	INT						DowngradeNetGeneration;
...
#ifdef UTPG_MD5
	FString RecalculateMD5();
#endif

	// stijn: reloads an export's _exact_ name from the package with the correct case
	UBOOL GetRawExportName(INT ExportIndex, FString& RawName);
...
private:
	...
	// Deletes the loader after a failed Load operation
	void DeleteLoader();
	...
	FArchive& operator<<( UObject*& Object )
	{
		...
		*Loader << AR_INDEX(Index);
		...
		memcpy(&Object, &Temporary, sizeof(UObject*));

		return *this;
		unguardf(( TEXT("(%s %i))"), *FObjectFullName(this), Tell() ));
	}
	...
	FArchive& operator<<( FName& Name )
	{
		...
		*Loader << AR_INDEX(NameIndex);
		...
		if ( LoadingExports )
			LastExportNameIndex = NameIndex;
		FName Temporary = NameMap( NameIndex );
		memcpy(&Name, &Temporary, sizeof(FName));
