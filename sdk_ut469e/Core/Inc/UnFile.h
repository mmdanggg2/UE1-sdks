...
#else
	#define debugfSlow(...)		((void)0)
	#define appErrorfSlow(...)	((void)0)
#endif
...
public:
	...
	// Create new language file if doesn't exist, and return filepath of it.
...
//
// If @DiscardIllegalSequences == true, the conversion function will (attempt to),
// discard illegal sequences in the input string if the underlying string conversion API
// supports discarding.
// Note: The Windows implementations of these functions always discard illegal sequences,
// regardles of the value of @DiscardIllegalSequences.
//
...
CORE_API INT appToAnsiInPlace(ANSICHAR* Dst, const TCHAR* Src, size_t MaxCount, bool DiscardIllegalSequences=false);
...
CORE_API INT appFromAnsiInPlace(TCHAR* Dst, const ANSICHAR* Src, size_t MaxCount, bool DiscardIllegalSequences=false);

// Converts C++ wchar to UTF-16LE
CORE_API INT appToUnicodeInPlace(UNICHAR* Dst, const TCHAR* Src, size_t MaxCount, bool DiscardIllegalSequences=false);
// Converts UTF-16LE to C++ wchar
CORE_API INT appFromUnicodeInPlace(TCHAR* Dst, const UNICHAR* Src, size_t MaxCount, bool DiscardIllegalSequences=false);

// Converts C++ wchar to UTF-8
CORE_API INT appToUtf8InPlace(ANSICHAR* Dst, const TCHAR* Src, size_t MaxCount, bool DiscardIllegalSequences=false);
// Converts UTF-8 to C++ wchar
CORE_API INT appFromUtf8InPlace(TCHAR* Dst, const ANSICHAR* Src, size_t MaxCount, bool DiscardIllegalSequences=false);
...
CORE_API UBOOL appStrtoUBOOL( const TCHAR* Str );
...
CORE_API UBOOL ParseDestructive( FString& Str, const TCHAR* Match, FString& Value );
...
//
...
#if OLDUNREAL_USE_MIMALLOC && !MOD_BUILD && !_DEBUG

#if defined(__cplusplus) && (__cplusplus >= 201703)
  #define CORE_mi_decl_nodiscard    [[nodiscard]]
#elif (defined(__GNUC__) && (__GNUC__ >= 4)) || defined(__clang__)  // includes clang, icc, and clang-cl
  #define CORE_mi_decl_nodiscard    __attribute__((warn_unused_result))
#elif defined(_HAS_NODISCARD)
  #define CORE_mi_decl_nodiscard    _NODISCARD
#elif (_MSC_VER >= 1700)
  #define CORE_mi_decl_nodiscard    _Check_return_
#else
  #define CORE_mi_decl_nodiscard
#endif

#if defined(_MSC_VER) || defined(__MINGW32__)
  #if defined(__MINGW32__)
    #define CORE_mi_decl_restrict
    #define CORE_mi_attr_malloc              __attribute__((malloc))
  #else
    #if (_MSC_VER >= 1900) && !defined(__EDG__)
      #define CORE_mi_decl_restrict          __declspec(allocator) __declspec(restrict)
    #else
      #define CORE_mi_decl_restrict          __declspec(restrict)
    #endif
    #define CORE_mi_attr_malloc
  #endif
  #define CORE_mi_attr_alloc_size(s)
  #define CORE_mi_attr_alloc_size2(s1,s2)
  #define CORE_mi_attr_alloc_align(p)
  ...
  #define CORE_mi_attr_malloc                __attribute__((malloc))
    ...
    #define CORE_mi_attr_alloc_size(s)       __attribute__((alloc_size(s)))
    #define CORE_mi_attr_alloc_size2(s1,s2)  __attribute__((alloc_size(s1,s2)))
    #define CORE_mi_attr_alloc_align(p)
  #else
    #define CORE_mi_attr_alloc_size(s)       __attribute__((alloc_size(s)))
    #define CORE_mi_attr_alloc_size2(s1,s2)  __attribute__((alloc_size(s1,s2)))
    #define CORE_mi_attr_alloc_align(p)      __attribute__((alloc_align(p)))
  #endif
#else
  #define CORE_mi_decl_restrict
  #define CORE_mi_attr_malloc
...
CORE_mi_decl_nodiscard CORE_mi_decl_restrict
CORE_API void* Core_mi_malloc_aligned(size_t size, size_t alignment, const TCHAR* Tag, const TCHAR* Func)
CORE_mi_attr_malloc CORE_mi_attr_alloc_size(1) CORE_mi_attr_alloc_align(2);

CORE_mi_decl_nodiscard
CORE_API void* Core_mi_realloc_aligned(void* p, size_t newsize, size_t alignment, const TCHAR* Tag)
CORE_mi_attr_alloc_size(2) CORE_mi_attr_alloc_align(3);

CORE_API void Core_mi_free_aligned(void* p, size_t alignment, const TCHAR* Func);
CORE_API void Core_mi_free_size(void* p, size_t size);
CORE_API void Core_mi_free_size_aligned(void* p, size_t size, size_t alignment);

// Bypass GMalloc and call mimalloc directly. Suitable for internal use only
# define appMalloc(Size, Tag)						Core_mi_malloc_aligned(Size, VECTOR_ALIGNMENT, Tag, TEXT("appMalloc"))
# define appMallocAligned(Size, Alignment, Tag)		Core_mi_malloc_aligned(Size, static_cast<size_t>(Alignment), Tag, TEXT("appMallocAligned"))
# define appFree(Ptr)								Core_mi_free_aligned(Ptr, VECTOR_ALIGNMENT, TEXT("appFree"))
# define appFreeSize(Ptr, Size)						Core_mi_free_size(Ptr, Size)
# define appFreeAligned(Ptr, Alignment)				Core_mi_free_aligned(Ptr, static_cast<size_t>(Alignment), TEXT("appFreeAligned"))
# define appFreeSizeAligned(Ptr, Size, Alignment)	Core_mi_free_size_aligned(Ptr, Size, static_cast<size_t>(Alignment))
# define appRealloc(Ptr, NewSize, Tag)				Core_mi_realloc_aligned(Ptr, NewSize, VECTOR_ALIGNMENT, Tag)
#else
// Allocation functions suitable for external mods that target multiple engine versions
# define appMalloc(Size, Tag)						GMalloc->Malloc(Size, Tag)
# define appMallocAligned(Size, Alignment, Tag)		GMalloc->Malloc(Size, Tag)
# define appFree(Ptr)								GMalloc->Free(Ptr)
# define appFreeSize(Ptr, Size)						GMalloc->Free(Ptr)
# define appFreeAligned(Ptr, Alignment)				GMalloc->Free(Ptr)
# define appFreeSizeAligned(Ptr, Size, Alignment)	GMalloc->Free(Ptr)
# define appRealloc(Ptr, NewSize, Tag)				GMalloc->Realloc(Ptr, NewSize, Tag)
#endif

//
// C++ memory allocation overrides. MSVC can inline them reliably.
// gcc and clang cannot. On Linux/macOS, we override C++ allocation
// operators using non-inlined functions in the main executable instead.
//
#if _MSC_VER && !defined(NO_APP_MALLOC) && !defined(UTGLR_NO_APP_MALLOC)
#define INLINE_OPERATORS 1
#include "MallocOverrides.h"
#endif

// This is a user-defined new operator that is always safe to inline
