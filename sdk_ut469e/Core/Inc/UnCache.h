...
public:
	...
	public:
		...
		{
#if DO_GUARD_SLOW
// stijn: calling a class method on a null pointer is undefined behavior in C++. The compiler will generally remove this check
//			if( this == NULL )
//				appErrorf( TEXT("Unlock: Null cache item") );
	...
	private:
		...
		FCacheItem*	FreeNext;		// Next free item in free list, or NULL if last.
		FCacheItem*	FreePrev;		// Previous free item in free list, or NULL if first.
	};
...
private:
	...
	BYTE*       CacheMemory;
	BYTE		StoreAtEnd[CID_COUNT] = {};
	FCacheItem* EmptyBlock;
	FCacheItem* CurrItem;
	FCacheItem* FreeItems;
	BYTE*       CacheMemoryEnd;
	...
	// Internal functions.
	void CreateNewFreeSpace( BYTE* Start, BYTE* End, FCacheItem* Prev, FCacheItem* Next, INT Segment, UBOOL bMergeEmpty = TRUE );
	...
	void RemoveFromFreeList(FCacheItem* Item);
