...
// Structure that links active stack frames. We use this to support 
// our implementation of the BACKTRACE console command. Adding a Prev 
// pointer to FFrame would have been cleaner, but ACE doesn't like that.
struct CORE_API FFrameTracker
{
	FFrame* Stack;
	FFrame*	PrevStack;
	FFrameTracker* Next;

	static FFrame* GetPrevStack(FFrame* InStack)
	{
		for (FFrameTracker* Curr = GFrameTracker; Curr; Curr = Curr->Next)
			if (Curr->Stack == InStack)
				return Curr->PrevStack;
		return NULL;
	}

	FFrameTracker(FFrame* InStack, FFrame* InPrevStack)
		: Stack(InStack)
		, PrevStack(InPrevStack)
	{
		Next = GFrameTracker;
		GFrameTracker = this;
	}
...
private:
	static FFrameTracker* GFrameTracker;
};
