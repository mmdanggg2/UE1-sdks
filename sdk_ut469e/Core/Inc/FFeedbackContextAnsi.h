...
public:
	...
	{
		guard(FFeedbackContextAnsi::Ask);
		if( (GIsClient || GIsEditor) && !ParseParam(appCmdLine(),TEXT("Silent")) )//!!
		{
			LocalPrint(*Temp);
			LocalPrint( *FString::Printf(TEXT(" ([Y]es/[N]o%ls): "), WithCancel ? TEXT("/[C]ancel") : TEXT("")) );
			fflush(stdout);
			fflush(stderr);
			...
			while ((Dummy = getchar()) != '\n' && Dummy != EOF)
				; 
			if (WithCancel && (Ch=='C' || Ch=='c'))
				return ANSWER_Cancel;
			return (Ch=='Y' || Ch=='y') ? ANSWER_Yes : ANSWER_No;
		}
		else return ANSWER_Yes;
		unguard;
	}
