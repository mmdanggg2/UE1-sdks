...
class UD3D9RenderDevice : public URenderDeviceOldUnreal469 {
	...
	inline void LockVertexColorBuffer(void) {
		...
		HRESULT hResult = m_d3dVertexColorBuffer->Lock(0, 0, (VOID **)&pData, lockFlags);
		if (FAILED(hResult)) {
	...
	inline void UnlockVertexColorBuffer(void) {
		HRESULT hResult = m_d3dVertexColorBuffer->Unlock();
		if (FAILED(hResult)) {
	...
	inline void LockSecondaryColorBuffer(void) {
		...
		HRESULT hResult = m_d3dSecondaryColorBuffer->Lock(0, 0, (VOID **)&pData, lockFlags);
		if (FAILED(hResult)) {
			appErrorf(TEXT("Vertex buffer lock failed: %ls"), *ExplainResult(hResult));
		}
	...
	inline void UnlockSecondaryColorBuffer(void) {
		HRESULT hResult = m_d3dSecondaryColorBuffer->Unlock();
		if (FAILED(hResult)) {
			appErrorf(TEXT("Vertex buffer unlock failed: %ls"), *ExplainResult(hResult));
		}
	}
	...
	inline void FASTCALL LockTexCoordBuffer(DWORD texUnit) {
		...
		HRESULT hResult = m_d3dTexCoordBuffer[texUnit]->Lock(0, 0, (VOID **)&pData, lockFlags);
		if (FAILED(hResult)) {
			appErrorf(TEXT("Vertex buffer lock failed: %ls"), *ExplainResult(hResult));
		}
	...
	inline void FASTCALL UnlockTexCoordBuffer(DWORD texUnit) {
		HRESULT hResult = m_d3dTexCoordBuffer[texUnit]->Unlock();
		if (FAILED(hResult)) {
			appErrorf(TEXT("Vertex buffer unlock failed: %ls"), *ExplainResult(hResult));
		}
	}
	...
	IDirect3DPixelShader9 *m_fpGammaShader;
	...
	// Shader gamma variables.
	IDirect3DTexture9* pRenderTargetTexture;
	IDirect3DSurface9* pRenderTargetSurface;
	IDirect3DSurface9* pOriginalRenderTarget;
	FGammaShaderVertex GammaShaderQuad[4];
	UBOOL DoShaderGamma;
	UBOOL UsedGammaRamp;
...
#undef BGRA_MAKE
	...
	void FASTCALL SC_AddByteConfigParam(const TCHAR* pName, BYTE& param, ECppProperty EC_CppProperty, INT InOffset, BYTE defaultValue, UEnum* Enum);
