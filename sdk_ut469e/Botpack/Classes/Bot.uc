...
function bool FindAmbushSpot()
{
	...
	if ( Ambushspot != None )
	{
		...
		{
			GoalString $= " reachable";
			MoveTarget = Ambushspot;
			return true;
		}
		GoalString $= " path there";
...
ignores SeePlayer, HearNoise;
	...
	*/
	...
	function bool StrafeFromDamage(vector momentum, float Damage, name DamageType, bool bFindDest)
	{
		...
		if (Enemy == None)
			sideDir = Normal( Normal(vect(0,0,0) - Location) Cross vect(0,0,1) );
		else
...
ignores EnemyNotVisible;
	...
	function bool ClearShot()
	{
		...
		if ( Enemy == None || !FastTrace(LastSeenPos + vect(0,0,0.9) * Enemy.CollisionHeight, Location) )
		{
			bFire = 0;
			bAltFire = 0;
			return false;
...
function Destroyed()
{
	...
	{
		for ( n=0; n<32; n++ )
			if ( DeathMatchPlus(Level.Game).BotConfig.BotNames[n] == PlayerReplicationInfo.PlayerName )
			{
				DeathMatchPlus(Level.Game).BotConfig.ConfigUsed[n] = 0;
				break;
			}
	}
	Super.Destroyed();
