...
var globalconfig bool bTranslucentCrosshair;
...
simulated function HUDSetup(canvas canvas)
{
	...
	SolidHUDColor = FavoriteHUDColor * 15.9375;
...
simulated function DrawBigNum(Canvas Canvas, int Value, int X, int Y, optional float ScaleFactor)
{
	...
	for (i = 0; i <= Nums; i++)
	{
		DrawDigit(Canvas, (Mag/d) % 10, Step, UpScale, bMinus);
		d = d/10;
	}
}
...
function bool DrawSpeechArea( Canvas Canvas, float XL, float YL )
{
	...
	local int paneltype, bgheight;
	local Texture SolidBG;
	...
	if ( PlayerOwner == None || PlayerOwner.Player == None || PlayerOwner.Player.Console == None || 
		!PlayerOwner.Player.Console.bTyping )
	{
		paneltype = 0;
		bgheight = 68;
	}
	...
	bgheight *= YPos/128;
	if (Opacity == 16 && MessageFadeTime == 1)
		SolidBG = Texture'FlakAmmoLEDBase';

	if (SolidBG != None)
	{
		Canvas.Style = ERenderStyle.STY_Normal;
		Canvas.SetPos(Yadj + FaceAreaOffset, 0);
		Canvas.DrawTile(SolidBG, 256*Scale - FaceAreaOffset, bgheight, 0, 0, SolidBG.USize, SolidBG.VSize);
		Canvas.Style = ERenderStyle.STY_Translucent;
	}
	Canvas.SetPos(Yadj + FaceAreaOffset, 0);
	...
	if (SolidBG != None)
	{
		...
		Canvas.DrawTile(SolidBG, WackNumber, bgheight, 0, 0, SolidBG.USize, SolidBG.VSize);
		Canvas.Style = ERenderStyle.STY_Translucent;
	}
	Canvas.SetPos(Yadj, 0);
	...
	if (SolidBG != None)
	{
		...
		Canvas.DrawTile(SolidBG, 64, bgheight, 0, 0, SolidBG.USize, SolidBG.VSize);
		Canvas.Style = ERenderStyle.STY_Translucent;
	}
	Canvas.SetPos(Yadj + WackNumber, 0);
...
simulated function PostRender( canvas Canvas )
{
	...
	if ( PlayerOwner.bShowScores || bForceScores )
	{
		...
		{ 
			...
			if ( PlayerOwner.Player != None && PlayerOwner.Player.Console != None && PlayerOwner.Player.Console.bTyping )
...
simulated function DrawCrossHair( canvas Canvas, int X, int Y)
{
	...
	CrosshairColor.R = Clamp(CrosshairColor.R, 0, 16);
	CrosshairColor.G = Clamp(CrosshairColor.G, 0, 16);
	CrosshairColor.B = Clamp(CrosshairColor.B, 0, 16);
	Canvas.DrawColor = 15.9375 * CrosshairColor;
