...
state Dropped
{
	...
	{
		local CTFFlag aFlag, otherFlag;
		local Pawn aPawn;
		local NavigationPoint N;
		local int num, i;
        local bool bCheckForInstantCap;
			...
			&& !aPawn.IsInState('FeigningDeath') )
		{
            ...
            // OldUnreal fix: cap instantly if whoever picks up this flag is also within the
            // collision radius of the home flagbase. In v436, you would have to pick up the
            // flag, walk out of the collision radius of the flag base, and walk back in to
            // score.
            if (bCheckForInstantCap &&
                CTFGame(Level.Game).bFixFlagBasePickup &&
                CTFReplicationInfo(CTFGame(Level.Game).GameReplicationInfo) != None &&
                aPawn.PlayerReplicationInfo.Team < ArrayCount(CTFReplicationInfo(CTFGame(Level.Game).GameReplicationInfo).FlagList))
            {				
                otherFlag = CTFReplicationInfo(CTFGame(Level.Game).GameReplicationInfo).FlagList[aPawn.PlayerReplicationInfo.Team];
				
                // Check if the player is touching their own flag
                foreach aPawn.TouchingActors( class'CTFFlag', aFlag )
                {
                    if (aFlag == otherFlag)
                    {
                        CTFGame(Level.Game).ScoreFlag(Pawn(Other), self);
                        SendHome();
                        return;
                    }
                }
