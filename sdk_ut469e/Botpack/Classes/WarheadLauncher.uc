...
{
	local vector Start, Offset, X,Y,Z;
	...
	Offset = CalcDrawOffset() + FireOffset.X * X + FireOffset.Y * Y + FireOffset.Z * Z;
	Start = Owner.Location + Offset;
	AdjustedAim = PawnOwner.AdjustAim(ProjSpeed, Start, AimError, True, bWarn);
	Proj = Spawn(ProjClass,,, Start,AdjustedAim);
	if ( Proj == None )
	{
		// Try reducing X first
		Start = Owner.Location + Offset - (FireOffset.X * 0.5 * X);
		Proj = Spawn(ProjClass,,, Start,AdjustedAim);
		// Reduce entire offset
		if ( Proj == None )
		{
			Start = Owner.Location + Offset * 0.25;
			Proj = Spawn(ProjClass,,, Start,AdjustedAim);
		}
	}
	return Proj;	
