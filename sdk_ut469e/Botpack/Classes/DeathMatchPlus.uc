...
// OldUnreal
var() globalconfig bool bSimulateKickers; // if true, the server will replicate kickers so clients can simulate them
...
function PostBeginPlay()
{
	...
	if (bSimulateKickers)
	{
        foreach Level.AllActors(class'Kicker', K)
		{
			// We can only do this for kickers whose KickVelocity will not lose precision due to vector compression
            if (K.Class == class'Kicker' &&
			    Abs(K.KickVelocity.X) < 65535.f &&
				Abs(K.KickVelocity.Y) < 65535.f &&
				Abs(K.KickVelocity.Z) < 65535.f)
			    K.bAlwaysRelevant = true;
        }
    }
}
...
function EndSpree(Pawn Killer, Pawn Other)
{
	...
	for ( P=Level.PawnList; P!=None; P=P.NextPawn )
		if ( TournamentPlayer(P) != None )
			TournamentPlayer(P).EndSpree(KillerPRI, Other.PlayerReplicationInfo);
}
...
function Killed(pawn killer, pawn Other, name damageType)
{
	...
	{
		if (Other != None && Other.PlayerReplicationInfo != None && Killer.PlayerReplicationInfo != None)
		{
...
function ChangeName(Pawn Other, string S, bool bNameChange)
{
	...
	While ( APlayer != None )
	{	
		if ( APlayer.bIsPlayer && APlayer.PlayerReplicationInfo != None && (APlayer.PlayerReplicationInfo.PlayerName~=S) )
		{
...
function string GetRules()
{
	...
	ResultSet $= "\\fraglimit\\"$FragLimit;
	Resultset $= "\\minplayers\\"$MinPlayers;
	Resultset $= "\\changelevels\\"$bChangeLevels;
	Resultset $= "\\tournament\\"$bTournament;
	if(bMegaSpeed)
		Resultset $= "\\gamestyle\\Turbo";
	else
	if(bHardcoreMode)
		Resultset $= "\\gamestyle\\Hardcore";
	else
		Resultset $= "\\gamestyle\\Classic";

	if(MinPlayers > 0)
		Resultset $= "\\botskill\\"$class'ChallengeBotInfo'.default.Skills[BotConfig.Difficulty];
