...
function SetClientFFireMessage(int messageIndex, PlayerReplicationInfo Recipient, out Sound MessageSound, out Float MessageTime)
{
	...
	DelayedResponse $= FFireString[messageIndex];
...
function SetClientTauntMessage(int messageIndex, PlayerReplicationInfo Recipient, out Sound MessageSound, out Float MessageTime)
{
	...
	DelayedResponse $= TauntString[messageIndex];
...
{
	DelayedResponse $= OrderString[messageIndex];
...
{
	DelayedResponse $= OtherString[messageIndex];
...
function Timer()
{
	...
	if ( bDelayedResponse )
	{
		...
		{
			if ( PlayerPawn(Owner).GameReplicationInfo.bTeamGame && DelayedSender != None
			...
			if ( PlayerPawn(Owner).GameReplicationInfo.bTeamGame )
			{
				if ( Callsign == -1 )
					...
					for ( P=Level.PawnList; P!=None; P=P.NextPawn )
						...
						if ( P.PlayerReplicationInfo != None && (P.PlayerReplicationInfo.TeamId == Callsign)
