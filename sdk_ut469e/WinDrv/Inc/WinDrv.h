...
public:
	...
	// Helper for filtering unwanted mouse moves and clicks in UnrealEd
	void UpdateCumulativeMovement(LONG Cumulative);
	...
	// Status variables that help us track (likely) unintended movement between mouse clicks
	DOUBLE LastNotSmallMoveTime{};
	DOUBLE LastIgnoredMoveTime{};
	INT CumulativeIgnoredMovement{};
};
...
public:
	...
	// Fields for filter stale movement after warp
	UBOOL PrevCaptureMouse{};
};
...
class DLL_EXPORT UWindowsClient : public UClient, public FNotifyHook
{
	...
	BITFIELD			UseDirectDrawBorderlessFullscreen;
...
class DLL_EXPORT UWindowsViewport : public UViewport
{
	...
	UBOOL IsFullscreen();
	UBOOL IsBorderless();
	void GetBorderlessSize(INT& Width, INT& Height);
