...
function bool InsertUnicode(string C)
{
	...
	NewValue = Left(Value, CaretOffset) $ C $ Mid(Value, CaretOffset);
		...
		if(bCanEdit)
		{
			if(bHistory)
			{
				if (Value != "")
				{
					...
					while (CurrentHistory != None)
					{
						if (CurrentHistory.Prev != None && UWindowEditBoxHistory(CurrentHistory.Prev).HistoryText == Value)
							CurrentHistory.Prev.Remove();
