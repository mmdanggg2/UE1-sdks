...
function TextAreaClipText(Canvas C, float DrawX, float DrawY, coerce string Text, optional bool bCheckHotkey)
{
	...
	Super.TextAreaClipText(C, DrawX, DrawY, Text, bCheckHotKey);
}
...
function string WriteStyleText(HTMLStyle CurrentStyle, int CharCount)
{
	...
	Output $= Left(Pad, 4 - Len(Temp)) $ Temp $ CurrentStyle.LinkDestination;

	Temp = string(CurrentStyle.TextColor.R);
	Output $= Left(Pad, 3 - Len(Temp)) $ Temp;
...
defaultproperties
{
	...
	BGColor=(R=0,G=0,B=0)
	LinkColor=(R=0,G=0,B=255)
	ALinkColor=(R=255,G=0,B=0)
