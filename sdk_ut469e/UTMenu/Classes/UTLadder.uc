...
function Created()
{
	...
	if ( bGOTY && (GOTYLadder != None) )
		LadderObj.CurrentLadder = GOTYLadder;
	else if (class'UTLadder'.Static.IsDemo())
		LadderObj.CurrentLadder = DemoLadder;
	else
		LadderObj.CurrentLadder = Ladder;
	log("Current Ladder is "$LadderObj.CurrentLadder);
	...
	C = LadderObj.CurrentLadder.Default.Matches;
	MaxBaseMatch = C - 8;
	...
	Matches[C-1].MyFont = class'UTLadderStub'.Static.GetStubClass().Static.GetBigFont(Root);
	...
	for (i=C-2; i>0; i--)
	{
		...
		Matches[i].MyFont = class'UTLadderStub'.Static.GetStubClass().Static.GetBigFont(Root);
	...
	Matches[0].MyFont = class'UTLadderStub'.Static.GetStubClass().Static.GetBigFont(Root);
...
function Notify(UWindowWindow B, byte E)
{
	...
	switch (E)
	{
		...
		case DE_Click:
			...
			{
				case Scrollup:
					if (LadderObj.CurrentLadder.Default.Matches > 8)
					{
						BaseMatch++;
...
function EvaluateMatch(optional bool bTrophyVictory)
{
	...
	if (LadderObj != None)
	{
		...
		SaveString $= "\\"$Team;
		SaveString $= "\\"$LadderObj.DMRank;
		SaveString $= "\\"$LadderObj.DMPosition;
		SaveString $= "\\"$LadderObj.DOMRank;
		SaveString $= "\\"$LadderObj.DOMPosition;
		SaveString $= "\\"$LadderObj.CTFRank;
		SaveString $= "\\"$LadderObj.CTFPosition;
		SaveString $= "\\"$LadderObj.ASRank;
		SaveString $= "\\"$LadderObj.ASPosition;
		SaveString $= "\\"$LadderObj.ChalRank;
		SaveString $= "\\"$LadderObj.ChalPosition;
		SaveString $= "\\"$LadderObj.Sex;
		SaveString $= "\\"$LadderObj.Face;
		SaveString $= "\\"$GetPlayerOwner().PlayerReplicationInfo.PlayerName;
