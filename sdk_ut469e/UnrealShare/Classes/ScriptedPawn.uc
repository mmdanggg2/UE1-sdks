...
var		bool		bAttitudeQuery;	// In the middle of a reciprocal 'AttitudeToCreature' check
...
// OldUnreal
var() config bool bTeamGamePlusFix; // See comment above IsTeamGame below
...
function bool SetEnemy( Pawn NewEnemy )
{
	...
	else if ( newAttitude == ATTITUDE_Friendly ) //your enemy is my enemy
	{
		...
		if ( !bIgnoreFriends && (NewEnemy.Enemy != None) && (NewEnemy.Enemy.Health > 0) ) 
		{
			result = true;
...
// OldUnreal: The AttitudeTo function below originally only considered subclasses of UnrealShare.TeamGame to be proper team games.
// This meant ScriptedPawns behaved differently in UT TeamGamePlus-based games.
// Setting bTeamGamePlusFix to true makes ScriptedPawns behave the same in _all_ team games.
function bool IsTeamGame()
{
    if (bTeamGamePlusFix)
	    return Level.Game.bTeamGame;
	return Level.Game.IsA('TeamGame');
}
...
function eAttitude AttitudeTo(Pawn Other)
{
	...
	if (Other.bIsPlayer)
	{
		if ( bIsPlayer && IsTeamGame() && (Other.PlayerReplicationInfo != none) && (Team == Other.PlayerReplicationInfo.Team) )
...
function eAttitude AttitudeToCreature(Pawn Other)
{
	...
	else if ( !bAttitudeQuery && (ScriptedPawn(Other) != None) )
	{
		// If other wants to be friendly, consider a friend.
		// If other doesn't consider me a friend, don't consider other as friend.
		ScriptedPawn(Other).bAttitudeQuery = true;
		Attitude = ScriptedPawn(Other).AttitudeToCreature(self);
		ScriptedPawn(Other).bAttitudeQuery = false;
...
// specialized for ScriptedPawns
function bool IsBlockedBy(Actor Other)
{
	if (Other == Level)
...
GoToGuard:
	if ( OrderObject != None && VSize(Location - OrderObject.Location) < 2 * CollisionRadius)
		Goto('Turn');
	TweenToRunning(0.2);
	FinishAnim();
	PlayRunning();
...
ignores SeePlayer, HearNoise, Bump;
	...
	function BeginState()
	{
		...
		else
		{
			...
			{
				if (Enemy != None && (Enemy == self || Enemy.bDeleteMe))
					Enemy = None;
				GotoState('Attacking');
			}
		}
...
CloseIn:
	...
	else
	{
		...
		if (MoveTarget == None)
		{
			AlarmTag = '';
			if (SpecialGoal != None)
				Log(self $ ": no path to alarm, SpecialGoal =" @ SpecialGoal);
			else
				Log(self $ ": no path to alarm, OrderObject =" @ OrderObject);
			GotoState('Attacking');
		}
		if ( SpecialPause > 0.0 )
		{
...
Wait:
	...
	FinishAnim();
	if ( Enemy != None
	    && (VSize(Location - Enemy.Location) > CollisionRadius + Enemy.CollisionRadius + 220) 
