...
public:
	...
	:	Type(InType), ArrayDim(1), ArrayDim2(0), PropertyFlags(0), Enum(NULL), MetaClass(NULL)
	{}
	FPropertyBase( UEnum* InEnum )
	:	Type(CPT_Byte), ArrayDim(1), ArrayDim2(0), PropertyFlags(0), Enum(InEnum), MetaClass(NULL)
	{}
	FPropertyBase( UClass* InClass, UClass* InMetaClass=NULL )
	:	Type(CPT_ObjectReference), ArrayDim(1), ArrayDim2(0), PropertyFlags(0), PropertyClass(InClass), MetaClass(InMetaClass)
	{}
	FPropertyBase( UStruct* InStruct )
	:	Type(CPT_Struct), ArrayDim(1), ArrayDim2(0), PropertyFlags(0), Struct(InStruct), MetaClass(NULL)
	{}
	FPropertyBase( UProperty* Property )
	{
		...
		{
			appErrorf( TEXT("Unknown property type '%s'"), *FObjectFullName(Property) );
		}
		ArrayDim = DynArray ? 0 : Property->ArrayDim;
		if ( ArrayProperty && ArrayProperty->StaticArrayDim > 1 )
			ArrayDim2 = ArrayProperty->StaticArrayDim;
	...
	{
		static constexpr INT ElementSizes[CPT_MAX] =
		{
			0,
			sizeof(BYTE),
			sizeof(INT),
	...
	{
		FString Out = TEXT("");
		if (PropertyFlags & CPF_Const)
			Out += TEXT("const ");
		if (PropertyFlags & CPF_OutParm)
			Out += TEXT("out ");
		switch (Type)
		{
			case CPT_None: Out += TEXT("None"); break;
			case CPT_Byte: Out += TEXT("byte"); break;
			case CPT_Int: Out += TEXT("int"); break;
			case CPT_Bool: Out += TEXT("bool"); break;
			case CPT_Float: Out += TEXT("float"); break;
			case CPT_ObjectReference:
				if (PropertyClass != UClass::StaticClass()) 
					Out += FObjectPathName(PropertyClass);
				else
					Out += FString::Printf(TEXT("%ls<%ls>"), *FObjectPathName(PropertyClass), *FObjectPathName(MetaClass));
				break;
			case CPT_Name: Out += TEXT("name"); break;
			case CPT_Struct: Out += FObjectPathName(Struct); break;
			case CPT_Vector: Out += TEXT("vector"); break;
			case CPT_Rotation: Out += TEXT("rotator"); break;
			case CPT_String: Out += TEXT("string"); break;
			case CPT_Pointer: Out += TEXT("pointer"); break;
			default: Out += TEXT("unknown"); break;
		}
		return Out;
	}
};
...
public:
	...
	void AttemptToConvertConstant( const FPropertyBase& NewType, FScriptCompiler* Compiler=nullptr )
	{
		...
		switch( NewType.Type )
		{
			...
			{
				BYTE V=0;
				if( NewType.Enum==NULL && GetConstByte(V, Compiler) )
					SetConstByte(NULL,V); 
				break;
			}
			case CPT_ObjectReference:
	...
	// Getters.
	UBOOL GetConstByte( BYTE& B, FScriptCompiler* Compiler = nullptr );
	UBOOL GetConstInt( INT& I, FScriptCompiler* Compiler = nullptr )
	{
	...
	UBOOL GetConstBool( UBOOL& B, FScriptCompiler* Compiler = nullptr )
	{
	...
	UBOOL GetConstFloat( FLOAT& R, FScriptCompiler* Compiler = nullptr )
	{
	...
	UBOOL GetConstObject( UClass* DesiredClass, UObject*& Ob, FScriptCompiler* Compiler = nullptr )
	{
	...
	UBOOL GetConstName( FName& n, FScriptCompiler* Compiler = nullptr )
	{
	...
	UBOOL GetConstVector( FVector& v, FScriptCompiler* Compiler = nullptr )
	{
	...
	//!!struct constants
	UBOOL GetConstRotation( FRotator& r, FScriptCompiler* Compiler = nullptr )
	{
...
public:
	...
	//
	// HUGE FAT WARNING FROM STIJN:
	// UEngine reserves 4 bytes to store UObject references in UnrealScript bytecode. 
	// This is problematic for 64-bit builds, where we need 8 bytes to store a full pointer.
	// UEngine3 uses an O(n) lookup to map UObject pointers to a 4-byte index. 
	// This is super expensive and should be avoided. I wonder if storing the 
	// GObjObjects index could work...
	//
	// UPDATE:
	// Did some quick experiments which seem to suggest that this would actually work...
	//
	FArchive& operator<<( class UObject*& Res )
		//{DWORD D = (DWORD)Res; return *this << D;}
	{
#if BUILD_64
		DWORD D = Res->GetIndex();
#else
		DWORD D = (DWORD)Res;
#endif
		return *this << D;
...
public:
	...
	FString GetContext(UBOOL bShort = FALSE);
	...
	UField*			FindImportedField( UStruct* InScope, const TCHAR* InIdentifier, UClass* FieldClass=UField::StaticClass(), const TCHAR* Thing=NULL );
