...
struct ENGINE_API FSceneNode
{
	...
	// Helper for Editor.
	void Init(UViewport* InViewport);
};
...
struct FTransform : public FOutVector
{
	...
	{
		FLOAT ClipXM, ClipXP, ClipYM, ClipYP;
		static constexpr BYTE OutXMinTab [2] = { 0, FVF_OutXMin };
		static constexpr BYTE OutXMaxTab [2] = { 0, FVF_OutXMax };
		static constexpr BYTE OutYMinTab [2] = { 0, FVF_OutYMin };
		static constexpr BYTE OutYMaxTab [2] = { 0, FVF_OutYMax };
...
//
// Describes the 2D coordinates of a renderable quad's points and UV mapping.
//
struct FTileRect
{
	FLOAT X, Y, XL, YL;
...
struct ENGINE_API FStaticBspInfoBase
{
	...
	// Definitions
	static constexpr QWORD AtlasID_And = ~0xE000000000000000ull;
	static constexpr QWORD AtlasID_Or  =  0xA000000000000000ull;
	...
	// Lightmap info
	TArray<FLightMapIndex> LightMaps;
	TArray<FLightMapIndex> AtlasMaps;
	...
	static UBOOL IsAtlasID( QWORD CacheID)
	{
		return ((CacheID & AtlasID_And) | AtlasID_Or) == CacheID;
	}
};
...
// Hit a Mover surface.
struct ENGINE_API HMoverSurf : public HHitProxy
{
	DECLARE_HIT_PROXY(HMoverSurf,HHitProxy)
	AActor* Actor;
	INT iBrushPoly; // Actor->Brush->Polys->Element index
	HMoverSurf(AActor* InActor, INT iInBrushPoly) : Actor(InActor), iBrushPoly(iInBrushPoly) {}
};
