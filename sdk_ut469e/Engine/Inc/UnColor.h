...
//
#define GET_COLOR_DWORD(color) (*(DWORD*)&(color))
class alignas(4) ENGINE_API FColor
{
public:
	// Variables.
...
public:
	...
	{}
	FColor( const FPlane& P )
	:	R(Clamp<INT>(appRound(P.X*255),0,255))
	, G(Clamp<INT>(appRound(P.Y*255),0,255))
	, B(Clamp<INT>(appRound(P.Z*255),0,255))
	, A(Clamp<INT>(appRound(P.W*255),0,255))
	{}
	FColor( const FColor32F& C );
...
{
	//debugf( TEXT("FSaturateScale( %s )"), *Color.String() );
