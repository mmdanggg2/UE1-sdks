...
{
	FQuat   		Orientation;  //
	FPackedVector	Position;     //  needed or not ?
...
struct FNamedBoneBinary
{
	...
	// Metallicafan212:	Serializer support
	friend FArchive& operator<<(FArchive& Ar, FNamedBoneBinary& B)
	{
		// Metallicafan212:	First the name
		Ar.Serialize(B.Name, ARRAY_COUNT(B.Name) * sizeof(ANSICHAR));
...
struct AnimInfoBinary
{
	...
	// Metallicafan212:	Serializer support
	friend FArchive& operator<<(FArchive& Ar, AnimInfoBinary& B)
	{
		// Metallicafan212:	First the name
		Ar.Serialize(B.Name, ARRAY_COUNT(B.Name) * sizeof(ANSICHAR));

		// Metallicafan212:	Now the group
		Ar.Serialize(B.Group, ARRAY_COUNT(B.Group) * sizeof(ANSICHAR));

		// Metallicafan212:	Now the rest
		return Ar << B.TotalBones << B.RootInclude << B.KeyCompressionStyle << B.KeyQuotum << B.KeyReduction << B.TrackTime << B.AnimRate << B.StartBone << B.FirstRawFrame << B.NumRawFrames;
	}
};
...
struct VChunkHeader
{
	...
	{
		// Metallicafan212:	First the Chunk name
		Ar.Serialize(H.ChunkID, ARRAY_COUNT(H.ChunkID) * sizeof(ANSICHAR));
...
struct VMaterial
{
	...
	{
		// Metallicafan212:	First the name
		Ar.Serialize(M.MaterialName, ARRAY_COUNT(M.MaterialName) * sizeof(ANSICHAR));

		// Metallicafan212:	Now the rest
		return Ar << M.TextureIndex << M.PolyFlags << M.AuxMaterial << M.AuxFlags << M.LodBias << M.LodStyle;
	}
};
...
struct VRawBoneInfluence // just weight, vertex, and Bone, sorted later....
{
	...
	// Metallicafan212:	Serializer support
	friend FArchive& operator<<(FArchive& Ar, VRawBoneInfluence& B)
	{
		return Ar << B.Weight << B.PointIndex << B.BoneIndex;
	}
};
...
struct VQuatAnimKey
{
	FPackedVector		Position;           // relative to parent.
...
{	
	...
	// Metallicafan212:	Serializer support
	friend FArchive& operator<<(FArchive& Ar, MotionChunkDigestInfo& M)
	{
		return Ar << M.Name << M.Group << M.RootInclude << M.KeyCompressionStyle << M.KeyQuotum << M.KeyReduction << M.TrackTime << M.AnimRate << M.StartBone << M.FirstRawFrame << M.NumRawFrames;
	}
};
...
struct MotionChunk
{
	FPackedVector RootSpeed3D;  // Net 3d speed.
...
struct VVertex
{
	...
	_WORD	Pad1;		// Metallicafan212: U is padded to byte 4
	...
	_WORD	Pad2;		// Metallicafan212:	Fix the end padding

	// Metallicafan212:	Serializer support
	friend FArchive& operator<<(FArchive& Ar, VVertex& V)
	{
		return Ar << V.PointIndex << V.Pad1 << V.U << V.V << V.MatIndex << V.Reserved << V.Pad2;
	}
};
...
struct VPoint
{	
	FPackedVector			Point; // Change into packed integer later IF necessary, for 3x size reduction...

	// Metallicafan212:	Serializer support
	friend FArchive& operator<<(FArchive& Ar, VPoint& V)
	{
		return Ar << V.Point;
	}
};
...
struct VTriangle
{
	...
	{
		return Ar << T.WedgeIndex[0] << T.WedgeIndex[1] << T.WedgeIndex[2] << T.MatIndex << T.AuxMatIndex << T.SmoothingGroups;
	}
};
...
struct USkelImport 
{
	...
	TArray <FPackedVector>		Points;				// 3D Points
...
class ENGINE_API USkeletalMesh : public ULodMesh
{
	...
	// Cache for outcoded skeleton.
	TArray <FCoords> SpaceBasesCache;
	// Cache for final multi-bone vertex deformation.
	TArray <FVector> OutVertsCache;
