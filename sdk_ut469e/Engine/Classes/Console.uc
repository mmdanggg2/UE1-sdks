...
function ClearCursor(optional bool bKeep)
{
	if (TypeCursorOffset > 0)
		TypedStr = Left(TypedStr, Max(0, Len(TypedStr) - TypeCursorOffset - 1)) $
			Right(TypedStr, TypeCursorOffset);
	if (!bKeep)
		TypeCursorOffset = 0;
}
function SetCursor(int Delta)
{
	TypeCursorOffset += Delta;
	if (TypeCursorOffset > 0)
		TypedStr = Left(TypedStr, Max(0, Len(TypedStr) - TypeCursorOffset)) $ "|" $
			Right(TypedStr, TypeCursorOffset);	
}
	...
	function bool KeyType( EInputKey Key )
	{
		...
		if( Key>=0x20 && Key<0x100 && Key!=Asc("~") && Key!=Asc("`") )
		{
			if (TypeCursorOffset == 0)
				...
				TypedStr = Left(TypedStr, Max(0, Len(TypedStr) - TypeCursorOffset - 1)) $ Chr(Key) $ 
					 Right(TypedStr, TypeCursorOffset + 1);
			Scrollback=0;
			return true;
		}
	}
	function bool KeyEvent( EInputKey Key, EInputAction Action, FLOAT Delta )
	{
		...
		if( Key==IK_V && bControlDown && Action == IST_Press )
		{
			if (TypeCursorOffset == 0)
				...
				TypedStr = Left(TypedStr, Max(0, Len(TypedStr) - TypeCursorOffset - 1)) $ 
					Viewport.Actor.PasteFromClipboard() $ Right(TypedStr, TypeCursorOffset + 1);
        }

		if (Action == IST_Press && (Key == IK_Enter || Key == IK_Up || Key == IK_Down))
		{
			if (HistoryBot < HistoryTop || HistoryBot >= HistoryTop + MaxHistory)
			{
				HistoryBot = 0;
				for (HistoryTop = 0; HistoryTop < MaxHistory; HistoryTop++)
					if (History[HistoryTop] != "")
						History[HistoryBot++] = History[HistoryTop];
				HistoryTop = 0;
				HistoryBot--;
			}
			HistoryCur = Clamp(HistoryCur, HistoryTop, HistoryBot);
		}
		if( Key==IK_Escape )
		{
			if( Scrollback!=0 )
		...
		{
			if( Len(TypedStr)>0 && TypeCursorOffset > 0 )
			{
				TypedStr = Left(TypedStr, Max(0, Len(TypedStr) - TypeCursorOffset)) $
					Right(TypedStr, TypeCursorOffset - 1);
				TypeCursorOffset--;
		...
		{
			ClearCursor(true);
			if (TypeCursorOffset < Len(TypedStr))
				TypeCursorOffset++;
			SetCursor(0);
		}
