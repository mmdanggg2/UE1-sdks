...
//
// Note from OldUnreal: This is the desired horizontal FOV Angle.
// The effective horizontal FOV Angle might be different because:
// 1) Certain effects (e.g., teleporters) can temporarily adjust your FOV
// 2) Hor+ fov scaling permanently increases your horizontal fov angle to prevent widescreen windows from stretching out your viewport
//
...
//
var() globalconfig bool bDisableMovementBuffering; // deprecated
...
//
// OldUnreal wide-screen support
//

// Determines how the game scales the horizontal FOV and places weapons and their muzzle flash in first person view.
// 0 - Maintains the original (possibly stretched) horizontal FOV and places/scales the weapon and its muzzle flash like in UT 436.
// > 0 - Enables Hor+ fov scaling and adjusts the muzzle flash and possibly the weapon position according to the viewport's aspect ratio and FOVAngle.
// 1 - Calculates the weapon/muzzle flash position using the narrow fov angle. This should match the look of UT 436.
// 2 - Calculates the weapon/muzzle flash position using the Hor+ fov angle, but possibly shifts them towards the viewport center depending the FOVAngle and WeaponFOVScale.
var globalconfig transient byte FOVFix;
...
var globalconfig transient float WeaponFOVScale;

// 4:3 FOV we are emulating
var globalconfig transient float DesiredNarrowFOV;

// Viewport dimensions we calculated our FOV for
var transient int StoredCanvasX, StoredCanvasY;
...
)
{
	...
	OldLoc = Location;
	bCanTeleport = false;
	bFound = SetLocation(NewLocation);
	bCanTeleport = true;
	...
	if ( Carried != None )
	{
		...
		CarriedDecoration.SetLocation(Location + CarriedDecoration.Location - OldLoc);
					...
					if ( (P != self) && (P.Velocity != vect(0,0,0)) && P.bBlockPlayers )
					{
						...
						if ( (Velocity Dot Dir > 0) && (P.Velocity Dot Dir > 0) )
						{
							...
							if ( VSize(P.Location - Location) < P.CollisionRadius + CollisionRadius + CurrentMove.Delta * GroundSpeed )
								P.MoveSmooth(PlayerReplicationInfo.Ping * 0.0005 * P.Velocity);
						}
					}
...
{
	if ( !bShowMenu && (Level.Pauser == PlayerReplicationInfo.PlayerName) && PlayerReplicationInfo.PlayerName != "" )
	{
...
{
	// OldUnreal FOV Fix
	if (FOVFix != 0 && DesiredNarrowFOV >= 80.0 && DesiredNarrowFOV < 130.0)
	{
		// We might need to recalculate our desired FOV
		if (Canvas.SizeX != StoredCanvasX ||
		    Canvas.SizeY != StoredCanvasY)
		{
			StoredCanvasX = Canvas.SizeX;
			StoredCanvasY = Canvas.SizeY;

			SetNarrowFOV(DesiredNarrowFOV);
		}
	}
...
ignores SeePlayer, HearNoise, Bump, TakeDamage;
	...
	function ProcessMove(float DeltaTime, vector NewAccel, eDodgeDir DodgeMove, rotator DeltaRot)
	{
		...
		Velocity = Normal(NewAccel) * CheatFlyingSpeed;
		AutonomousPhysics(DeltaTime);
	}
...
exec function SetNarrowFOV(float FOV)
{
    ...
    local int i;
    local float ScreenWidth, ScreenHeight, EffectiveFOV;

	if (StoredCanvasX == 0 || StoredCanvasY == 0)
	{
	...
	{
		ScreenWidth  = StoredCanvasX;
		ScreenHeight = StoredCanvasY;
	}
	
    if (ScreenWidth > 0 && ScreenHeight > 0)
	{
	    DesiredNarrowFOV = FOV;
		EffectiveFOV = Atan(Tan(FClamp(FOV, 1.0, 179.0) * Pi / 360.0) *
                       FMax(1.0, 0.75 * ScreenWidth / ScreenHeight)) * 360.0 / Pi;
        SetDesiredFOV(EffectiveFOV);
	}    
}
...
defaultproperties
{
	 ...
	 CheatFlyingSpeed=300.000000
	 FOVFix=0
	 WeaponFOVScale=0.0
	 DesiredNarrowFOV=90.0
}
