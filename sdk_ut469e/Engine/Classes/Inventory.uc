...
simulated final function vector CalcDrawOffset()
{
    ...
    if (PawnOwner != None)
    {
        ...
        {
            FOVFix = class'PlayerPawn'.default.FOVFix;
            WeaponFOVScale = FClamp(class'PlayerPawn'.default.WeaponFOVScale, 0.0, 1.0);
 
            if (FOVFix == 0) // UT 436 method
                DrawOffset = ((0.9 / PawnOwner.FOVAngle * PlayerViewOffset) >> PawnOwner.ViewRotation);
            else if (Level.NetMode == NM_DedicatedServer || 
            	(FOVFix == 2 && WeaponFOVScale == 0.0) || PlayerPawn(Owner) == None || 
            	PlayerPawn(Owner).Player == None || PlayerPawn(Owner).Player.Console == None)
            ...
            else
            {
                ...
                else // mode 2
                {
                    ...
                    DrawOffset.X *= 1.0 - (1.0 - FOVScale) * WeaponFOVScale;
