...
{
	Shield = Spawn(class'QueenShield',,,Location + 150 * DrawScale * Vector(Rotation));
	if (Shield != None)
	{
		if (self.default.DrawScale != DrawScale)
		{
			Shield.DrawScale = DrawScale;
			Shield.SetCollisionSize(Shield.CollisionRadius * DrawScale, Shield.CollisionHeight * DrawScale);
		}
		Shield.SetBase(self);
	}
}
...
function bool CanTeleportInMap()
{
	...
	foreach AllActors(class'QueenDest', Nav)
		if (VSize(Location - Nav.Location) > CollisionRadius*2)
			return true;
	return false;
}
...
//	 ReducedDamageType='shot' // OldUnreal: this should have been: ReducedDamageType=shot. See UnrealTournamentPatches/issues/641
