...
function CreateColumns()
{
	...
	Ping	= AddColumn(PingName, 25);
	MapName	= AddColumn(MapNameName, 100);
	Players	= AddColumn(PlayersName, 40);
...
function DrawCell(Canvas C, float X, float Y, UWindowGridColumn Column, UBrowserServerList List)
{
	...
	case Ping:
		...
		break;
	case MapName:
		Column.ClipText( C, X, Y, List.MapName );
		break;
	case Players:
		if (List.NumPlayers == List.ReportedPlayersCount)
			Column.ClipText( C, X, Y, List.NumPlayers$"/"$List.MaxPlayers );
		else
			Column.ClipText( C, X, Y, List.NumPlayers$"("$List.ReportedPlayersCount$")/"$List.MaxPlayers );
		break;
	}
}
...
function KeyDown(int Key, float X, float Y) 
{
	...
	case 0x0D: // IK_Enter:
		...
		break;
	case 0x2E: // IK_Delete:
		if (SelectedServer != None && UBrowserFavoriteServers(GetParent(class'UBrowserServerListWindow')) != None)
		{
			i = GetSelectedRow();
			UBrowserFavoriteServers(GetParent(class'UBrowserServerListWindow')).RemoveFavorite(SelectedServer);
			SelectRow(Clamp(i, 0, Count - 2));
			VertSB.Show(GetSelectedRow());
		}
		break;
	default:
