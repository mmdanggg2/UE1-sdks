...
function Resolved( IpAddr Addr )
{
	...
	{
		Log( "UBrowserGSpyLink["$MasterServerAddress$":"$MasterServerTCPPort$"]: Invalid master server address, aborting." );
		return;
	}
	...
	if( BindPort() == 0 )
	{
		Log( "UBrowserGSpyLink["$MasterServerAddress$":"$MasterServerTCPPort$"]: Error binding local port, aborting." );
		return;
	}
...
event Timer()
{
	if(!bOpened)
	{
		Log("UBrowserGSpyLink["$MasterServerAddress$":"$MasterServerTCPPort$"]: Couldn't connect to master server.");
...
{
	Log("UBrowserGSpyLink["$MasterServerAddress$":"$MasterServerTCPPort$"]: Failed to resolve master server address, aborting.");
...
{
	Log("UBrowserGSpyLink["$MasterServerAddress$":"$MasterServerTCPPort$"]: Timed out in master server protocol.  Waiting for "$WaitingFor$" in state "$MatchData);
