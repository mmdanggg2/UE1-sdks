...
var localized string			SpectatorCountLeader;
...
var localized string			SpectatorCountName;
...
function WindowShown()
{
	...
	{
		VSplitter.SplitPos = GetSplitPosPercent()*VSplitter.WinHeight;
...
function float GetSplitPosPercent()
{
	local float SplitPosPercent;
	SplitPosPercent = UBrowserMainClientWindow(GetParent(class'UBrowserMainClientWindow')).SplitPosPercent;
	if (SplitPosPercent == 0.0)
		SplitPosPercent = 0.5;
	return FClamp(SplitPosPercent, 0.1, 0.9);
}
...
function QueryFinished(UBrowserServerListFactory Fact, bool bSuccess, optional string ErrorMsg)
{
	...
	if(!bSuccess)
	{
		...
		if(bDone && UnpingedList.Count() == 0 && !UnpingedList.bPinged)
		{
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	if (VSplitter.bWindowVisible)
	{
		SplitPosPercent = float(VSplitter.SplitPos)/VSplitter.WinHeight;
		if (UBrowserMainClientWindow(GetParent(class'UBrowserMainClientWindow')).SplitPosPercent != SplitPosPercent)
		{
			UBrowserMainClientWindow(GetParent(class'UBrowserMainClientWindow')).SplitPosPercent = SplitPosPercent;
			UBrowserMainClientWindow(GetParent(class'UBrowserMainClientWindow')).SaveConfigs();
		}
	}
	...
	case PS_Pinging:
	case PS_RePinging:
		W.DefaultStatusBarText(PingingText @ PercentComplete $ "% " $ CompleteText $ ". " $ 
			ServerCountLeader $ MyServers @ ServerCountName $ ", " $ 
			PlayerCountLeader $ PingedList.TotalPlayers @ PlayerCountName $ ", " $
			SpectatorCountLeader $ PingedList.TotalSpectators @ SpectatorCountName);
		break;
	case PS_Done:
		W.DefaultStatusBarText(ServerCountLeader $ 
			MyServers @ ServerCountName $ ", " $ 
			PlayerCountLeader $ PingedList.TotalPlayers @ PlayerCountName $ ", " $
			SpectatorCountLeader $ PingedList.TotalSpectators @ SpectatorCountName);
		break;
	}
...
defaultproperties
{
	...
	ServerCountLeader=""
	SpectatorCountName="Spectators"
	SpectatorCountLeader=""
