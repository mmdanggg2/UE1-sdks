...
{
	local string FirstMap, NextMap, TestMap, TutorialMap, AddMap;
	local int Selected;
	...
	TutorialMap = BotmatchParent.GameClass.Default.MapPrefix $ "-tutorial";
	while (!(FirstMap ~= TestMap))
	{
		// Add the map.
		AddMap = Left(NextMap, Len(NextMap) - 4);
		if(!(AddMap ~= TutorialMap))
			MapCombo.AddItem(AddMap, NextMap);
