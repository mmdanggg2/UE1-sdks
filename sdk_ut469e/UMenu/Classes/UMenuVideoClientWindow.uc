...
var localized string AdvancedVideoSettingsTitle;

// WindowMode
var UWindowComboControl WindowModeCombo;
var localized string WindowModeText;
var localized string WindowModeHelp;
var localized string WindowModes[3];
...
// Fov Mode
var UWindowComboControl FovModeCombo;
var localized string FovModeText;
var localized string FovModeHelp;
var localized string FovModes[3];
...
function Created()
{
    ...
    bRelaunchSupported = GetPlayerOwner().ConsoleCommand("RELAUNCHSUPPORT") ~= "ENABLED";
	if (bRelaunchSupported)
	{
	...
	// Advanced
	if ( VideoDriverDesc != "VideoDriverClassName" )
	{
		if (bRelaunchSupported)
		{
	        AdvancedButton = UWindowSmallButton(CreateControl(class'UWindowSmallButton', ControlRight + 49, ControlOffset - 25, 48, 16));
		}
		else
		{
		    AdvancedButton = UWindowSmallButton(CreateControl(class'UWindowSmallButton', ControlRight, ControlOffset - 2, 48, 16));
			ControlOffset += 23;
		}
	    AdvancedButton.WinHeight = 18;
	...
	// WindowMode
	WindowModeCombo = UWindowComboControl(CreateControl(class'UWindowComboControl', CenterPos, ControlOffset, CenterWidth, 1));
	WindowModeCombo.SetText(WindowModeText);
	WindowModeCombo.SetHelpText(WindowModeHelp);
	WindowModeCombo.SetFont(F_Normal);
	WindowModeCombo.SetEditable(False);
	ControlOffset += 25;

	WindowModeCombo.AddItem(WindowModes[0], "Full Screen");
	WindowModeCombo.AddItem(WindowModes[1], "Windowed");
	WindowModeCombo.AddItem(WindowModes[2], "Borderless");
	...
	// FovAngle
	FovAngleEdit = UWindowEditControl(CreateControl(class'UWindowEditControl', CenterPos, ControlOffset, CenterWidth, 1));
	...
	// FovMode
	FovModeCombo = UWindowComboControl(CreateControl(class'UWindowComboControl', CenterPos, ControlOffset, CenterWidth, 1));
	FovModeCombo.SetText(FovModeText);
	FovModeCombo.SetHelpText(FovModeHelp);
	FovModeCombo.SetFont(F_Normal);
	FovModeCombo.SetEditable(False);
	ControlOffset += 25;
	
	FovModeCombo.AddItem(FovModes[0], "Stretch (default)");
	FovModeCombo.AddItem(FovModes[1], "Hor+");
...
function LoadAvailableSettings()
{
	...
	ParseString = GetPlayerOwner().ConsoleCommand("GetScreenMode");
	if (ParseString == "Fullscreen")
	    WindowModeCombo.SetValue(WindowModes[0]);
	else if (ParseString == "Windowed")
	    WindowModeCombo.SetValue(WindowModes[1]);
	else
		WindowModeCombo.SetValue(WindowModes[2]);
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	if ( AdvancedButton != none )
	{
		AdvancedButton.AutoWidth(C);
		AdvancedButton.WinLeft = CenterPos + CenterWidth - 100;
		if (bRelaunchSupported)
		    AdvancedButton.WinLeft += 49;
	}

	WindowModeCombo.SetSize(CenterWidth, 1);
	WindowModeCombo.WinLeft = CenterPos;
	WindowModeCombo.EditBoxWidth = 100;
	...
	FovAngleEdit.SetSize(CenterWidth, 1);
	FovAngleEdit.WinLeft = CenterPos;
	FovAngleEdit.EditBoxWidth = 100;

	FovModeCombo.SetSize(CenterWidth, 1);
	FovModeCombo.WinLeft = CenterPos;
	FovModeCombo.EditBoxWidth = 100;
...
{
	local string VideoDriverClassName;
	local int i;
	local UMenuAdvancedDriverWindow DW;
	...
	class'UMenuAdvancedDriverClientWindow'.static.SetNextDriver(VideoDriverClassName, ConfirmSettingsRestartTitle, ConfirmSettingsRestartText);
	DW = UMenuAdvancedDriverWindow(Root.CreateWindow(class'UMenuAdvancedDriverWindow', 100, 100, 100, 100, self));
	DW.WindowTitle = AdvancedVideoSettingsTitle;
}

function WindowModeChanged()
{
    if(!bInitialized)
	    return;

	if (WindowModeCombo.GetValue() == WindowModes[0])
        GetPlayerOwner().ConsoleCommand("SetScreenMode Fullscreen");
	else if (WindowModeCombo.GetValue() == WindowModes[1])
	    GetPlayerOwner().ConsoleCommand("SetScreenMode Windowed");
	else if (WindowModeCombo.GetValue() == WindowModes[2])
	    GetPlayerOwner().ConsoleCommand("SetScreenMode Borderless");
...
{
    if (GetPlayerOwner().FOVFix > 0)
	{
        FovAngleEdit.SetValue(string(GetPlayerOwner().DesiredNarrowFOV));
		FovModeCombo.SetValue(FovModes[1]);
    }
	else
	{
        FovAngleEdit.SetValue(string(GetPlayerOwner().DesiredFOV));
		FovModeCombo.SetValue(FovModes[0]);
    }
}

function FovSettingsChanged()
{
    if (FovModeCombo.GetValue() == FovModes[0])
	{
        GetPlayerOwner().FOVFix = 0;
		GetPlayerOwner().SetDesiredFOV(float(FovAngleEdit.GetValue()));
    }
	else
	{
        GetPlayerOwner().FOVFix = 2;
		GetPlayerOwner().WeaponFOVScale = 0.67;
		GetPlayerOwner().SetNarrowFOV(float(FovAngleEdit.GetValue()));
    }
}
...
defaultproperties
{
	...
	AdvancedButtonHelp="Press this button to open the Advanced Video Driver Settings window."
	AdvancedVideoSettingsTitle="Advanced Video Driver Settings"
	...
	WindowModeHelp="Adjust the game window type."
	WindowModes(0)="Full Screen"
	WindowModes(1)="Windowed"
	WindowModes(2)="Borderless"
	FovAngleText="FOV Angle"
	FovAngleHelp="Horizontal FOV (field of view) angle in degrees."
	FovModeText="FOV Scaling Mode"
    FovModeHelp="Adjust the FOV (field of view) scaling mode. Stretch (default) = Original UT scaling mode. Hor+ = Expand FOV based on aspect ratio."
	FovModes(0)="Stretch (default)"
	FovModes(1)="Hor+"
}
