...
//
class DLL_EXPORT UD3DRenderDevice : public URenderDeviceOldUnreal469
{
    DECLARE_CLASS(UD3DRenderDevice,URenderDeviceOldUnreal469,CLASS_Config,D3DDrv)
	...
	void Unlock( BOOL Blit )
	{
		...
		if( Blit )
		{
			if( ViewportFullscreen )
			{
				...
				PrimarySurface7->Flip( NULL, DDFLIP_WAIT | (UseVSync?0:DDFLIP_NOVSYNC) );
			}
			else
			{
				POINT Point={0,0};
		...
		if (!ti)
		{
					...
					|| (VSize = Info.Mips[FirstMip]->VSize) > fi->PixelFormat->MaxHeight)
			{
				if (++FirstMip >= (DWORD)Info.NumMips)
				{
					...
					FirstMip = 0;
					Info.NumMips = 1;
					debugf(TEXT("D3D: Encountered oversize texture without sufficient mipmaps. Uploading a white texture instead of %ls"), *FObjectPathName(Info.Texture));
					break;
				}
			}
				...
				Info.VSize != 1 << Info.Mips[0]->VBits)
			{
				...
				debugf(TEXT("D3D: NPOT Textures are not supported. Uploading a white texture instead of %ls"), *FObjectPathName(Info.Texture));
			}
	...
	UBOOL SetRes( INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )
	{
		...
		debugf(NAME_Init,TEXT("Unreal Tournament Direct3D support - internal revision 1.9b"));
						...
						if( Pool )
						{
							...
							for( INT c=0; c<Copies && Pool->TexCount<Pool->DesiredSwapCopies; c++ )
							{
								if( VideoMemoryFull || !CreateVideoTexture(Pool) )
								{
									...
									{
										if (Count == 0)
											debugf(TEXT("Failed to preallocate initial textures, %ix%i: %s"), USize, VSize, *D3DError(h));
									}
								}
							}
						}
