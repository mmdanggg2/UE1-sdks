...
function string ParseQuery( IpAddr Addr, coerce string Query, int QueryNum, out int PacketNum )
{
  ...
  {
    if( Level.Game.bTeamGame && bTeamInfo && TeamGamePlus(Level.Game) != None )
    {
...
function bool SendAPacket(IpAddr Addr, int QueryNum, out int PacketNum, int bFinalPacket)
{
	...
	ReplyData $= "\\queryid\\"$QueryNum$"."$++PacketNum;
	if (bFinalPacket == 1) {
		ReplyData $= "\\final\\";
	}
...
function string GetBasic() {
	...
	ResultSet $= "\\gamever\\"$Level.EngineVersion;

	// Append revision.
	ResultSet $= "\\gamerevision\\"$Level.EngineRevision;

	// The most recent network compatible version.
	if( MinNetVer >= Int(Level.MinNetVersion) && 
		MinNetVer <= Int(Level.EngineVersion) )
		ResultSet $= "\\minnetver\\"$string(MinNetVer);
	else
		ResultSet $= "\\minnetver\\"$Level.MinNetVersion;

	// The regional location of this game.
	ResultSet $= "\\location\\"$Level.Game.GameReplicationInfo.Region;
...
function string GetInfo() {
	...
	//ResultSet $= "\\shortname\\"$Level.Game.GameReplicationInfo.ShortName;

	// The server port.
	ResultSet $= "\\hostport\\"$Level.Game.GetServerPort();

	// (optional) The server IP
	// if (ServerIP != "")
	//	ResultSet $= "\\hostip\\"$ServerIP;

	// The map/level title
	ResultSet $= "\\maptitle\\"$StrReplace(Level.Title, "\\", "");
	
	// Map name
	ResultSet $= "\\mapname\\"$Level.Outer.Name;

	// The mod or game type
	ResultSet $= "\\gametype\\"$Level.Game.Class.Name;

	// The number of players
	ResultSet $= "\\numplayers\\"$Level.Game.NumPlayers;

	// The maximum number of players
	ResultSet $= "\\maxplayers\\"$Level.Game.MaxPlayers;

	// The game mode: openplaying
	ResultSet $= "\\gamemode\\openplaying";
...
function string GetRules()
{
	...
	if( Level.Game.GameReplicationInfo.AdminName != "" )
		ResultSet $= "\\AdminName\\"$StrReplace(Level.Game.GameReplicationInfo.AdminName, "\\", "");
  ...
  if(Level.Game.EnabledMutators != "" && (InStr(ResultSet,"mutators") == -1))
    ResultSet $= "\\mutators\\"$Level.Game.EnabledMutators;
	return ResultSet;
}
...
function string GetPlayer( PlayerPawn P, int PlayerNum )
{
	...
	// Ping
	ResultSet $= "\\ping_"$PlayerNum$"\\"$P.ConsoleCommand("GETPING");

	// Team
	ResultSet $= "\\team_"$PlayerNum$"\\"$P.PlayerReplicationInfo.Team;

	// Class
	ResultSet $= "\\mesh_"$PlayerNum$"\\"$P.Menuname;

	// Skin
	if(P.Skin == None)
	{
		P.static.GetMultiSkin(P, SkinName, FaceName);
		ResultSet $= "\\skin_"$PlayerNum$"\\"$SkinName;
		ResultSet $= "\\face_"$PlayerNum$"\\"$FaceName;
	}
	else
	{
		ResultSet $= "\\skin_"$PlayerNum$"\\"$string(P.Skin);
		ResultSet $= "\\face_"$PlayerNum$"\\None";
	}
...
function string GetTeam( TeamInfo T, int TeamNum )
{
  ...
  //score
  ResultSet $= "\\score_"$TeamNum$"\\"$T.Score;
  //size
  ResultSet $= "\\size_"$TeamNum$"\\"$T.Size;
  return ResultSet;
}
