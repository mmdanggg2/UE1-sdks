...
{
	RENDERTARGET_System,
	RENDERTARGET_Swapchain,
	RENDERTARGET_Offscreen,
	RENDERTARGET_Offscreen_MSAA,
};

//Color modes for framebuffers
...
class UOpenGLRenderDevice : public URenderDevice {
	...
	static FGL::FPalettePool PalettePool;
...
//	UBOOL SetGLES( void* Window);
	...
	void FASTCALL SC_AddByteConfigParam(const TCHAR *pName, BYTE &param, ECppProperty EC_CppProperty, INT InOffset, BYTE defaultValue, UEnum* Enum=nullptr);
	...
	bool FASTCALL SetPixelFormat(INT NewColorBytes);
#endif
...
//	void DrawStaticBspSurf( INT iSurf, FSurfaceInfo& Surface) {};
	void DrawTileList(const FSceneNode* Frame, const FTextureInfo& Info, const FTileRect* Tiles, INT NumTiles, FSpanBuffer* Span, FLOAT Z, FPlane Color, FPlane Fog, DWORD PolyFlags);
	...
	void (UOpenGLRenderDevice::*m_pDrawTileList)(const FSceneNode*,FSurfaceInfo_DrawTile&,const FTileRect*,INT,FLOAT);
	...
	void DrawTileList_ARB(const FSceneNode* Frame, FSurfaceInfo_DrawTile& Surface, const FTileRect* TileList, INT TileCount, FLOAT Z);
...
//	void BlitFramebuffer_ARB();
	...
	void DrawTileCompute_GLSL3(FSceneNode* Frame, FSurfaceInfo_DrawTile& Surface, FLOAT X, FLOAT Y, FLOAT XL, FLOAT YL, FLOAT U, FLOAT V, FLOAT UL, FLOAT VL, FLOAT Z);
	...
	void DrawTileList_GLSL3(const FSceneNode* Frame, FSurfaceInfo_DrawTile& Surface, const FTileRect* TileList, INT TileCount, FLOAT Z);
	void DrawTileListCompute_GLSL3(const FSceneNode* Frame, FSurfaceInfo_DrawTile& Surface, const FTileRect* TileList, INT TileCount, FLOAT Z);
	...
	void ExecDraw_ComplexSurfaceALT_GLSL3( FGL::Draw::Command* Draw);
	...
	ERenderTarget                     PrimaryRenderTarget;  // Render target the renderer writes to, owns the depth buffer
	...
	struct FOffscreen
	{
		...
		FOpenGLRenderTarget RenderTarget[Buffers]; // Optional
		FOpenGLRenderTarget RenderTarget_MSAA[Buffers];
		FOpenGLRenderTarget RenderTarget_Swapchain[Buffers]; // Screen swapchain
		static constexpr INT TotalBuffers = Buffers * 3;

		FOpenGLFBOAttachment_Texture ColorAttachment[Buffers];
		FOpenGLFBOAttachment_RenderBuffer ColorAttachment_MSAA[Buffers];
		FOpenGLFBOAttachment_RenderBuffer DepthAttachment[Buffers];

		bool IsActive()                { return DepthAttachment[0].IsValid(); }
		bool IsPresentingToSwapchain() { return RenderTarget_Swapchain[0].IsValid(); }
		bool IsMSAA()                  { return RenderTarget_MSAA[0].IsValid(); }
	} Offscreen;
	...
	struct FBuffers
	{
		FLightMapParams_TBO LightMapParams;
	};
	static FBuffers Buffers;
	...
	inline bool UsingFBOMultisampling()
	{
		return Offscreen.IsActive() && Offscreen.IsMSAA();
	}
