...
static const TCHAR* RenderPathName = TEXT("");
...
#define CPP_PROPERTY_LOCAL(_name) _name, CPP_PROPERTY(_name)
	...
	SC_AddBoolConfigParam(0,  TEXT("AlwaysMipmap"), CPP_PROPERTY_LOCAL(AlwaysMipmap), 0);
	SC_AddBoolConfigParam(0,  TEXT("UsePrecache"), CPP_PROPERTY_LOCAL(UsePrecache), 0);
	SC_AddBoolConfigParam(0,  TEXT("UseTrilinear"), CPP_PROPERTY_LOCAL(UseTrilinear), 1);
	SC_AddBoolConfigParam(0,  TEXT("UseHDTextures"), CPP_PROPERTY_LOCAL(UseHDTextures), UTGLR_DEFAULT_UseHDTextures);
	...
	SC_AddByteConfigParam(TEXT("FramebufferBpc"), CPP_PROPERTY_LOCAL(FramebufferBpc), FB_BPC_10bit, FramebufferBpcEnum);
...
void UOpenGLRenderDevice::SC_AddByteConfigParam(const TCHAR* pName, BYTE& param, ECppProperty EC_CppProperty, INT InOffset, BYTE defaultValue, UEnum* Enum) {
	param = defaultValue;
...
bool UOpenGLRenderDevice::DetectContextType( DWORD PreferredContextType)
{
	...
	{
		if (!FOpenGLBase::InitProcs(true) )
			return false;
		FOpenGLBase::InitCapabilities(true);
		FOpenGLBase::InitInterfaces();
		if ( !InitGL1() )
			return false;
	...
	// If hardware supports at least OpenGL 4.3, prefer GL Core over ES and compatibility
	if ( DefaultMajor >= 4 && DefaultMinor >= 3 )
	{
		ContextTypes[1] = CONTEXTTYPE_GL3;
		ContextTypes[2] = CONTEXTTYPE_GLES;
		ContextTypes[3] = CONTEXTTYPE_GL1;
	}
#endif
	...
	if ( GL_DEV )
		ContextFlags |= FGL::EContextFlags::FLAGS_DEBUG;
	FOpenGLBase::SetContextAttribute( FGL::EContextAttribute::FLAGS, ContextFlags);
	...
	for ( INT i=0; i<TryContext.Num(); i++)
	{
		...
		else if ( TryContext(i) == CONTEXTTYPE_GLES )
		{
			...
			FOpenGLBase::SetContextAttribute( FGL::EContextAttribute::MAJOR_VERSION, 3);
			FOpenGLBase::SetContextAttribute( FGL::EContextAttribute::MINOR_VERSION, 1);
		}
		else
			continue; //?
...
UBOOL UOpenGLRenderDevice::SetRes(INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen)
{
	...
	{
		GLint ContextFlags;
		FOpenGLBase::glGetIntegerv(GL_CONTEXT_FLAGS, &ContextFlags);
		if ( ContextFlags & GL_CONTEXT_FLAG_DEBUG_BIT )
		{
			debugf(NAME_Init, TEXT("Created a debug OpenGL context"));
			FOpenGLBase::InstallDebugCallback();
		}
	}
...
void UOpenGLRenderDevice::ConfigValidate_Main(void) {
	...
	FramebufferBpc = Clamp<BYTE>(FramebufferBpc, FB_BPC_8bit, FB_BPC_16bit);
...
UBOOL UOpenGLRenderDevice::Init(UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen)
{
	...
	if ( Section )
	{
		...
		R += Section->Remove(TEXT("Translucency"));
		R += Section->Remove(TEXT("TextureClampHack"));
		R += Section->Remove(TEXT("DisableSpecialDT"));
		R += Section->Remove(TEXT("MaxLogTextureSize"));
		R += Section->Remove(TEXT("MinLogTextureSize"));
		R += Section->Remove(TEXT("MaxLogVOverU"));
		R += Section->Remove(TEXT("MaxLogUOverV"));
		R += Section->Remove(TEXT("UseVertexSpecular"));
		R += Section->Remove(TEXT("UseCVA"));
		R += Section->Remove(TEXT("UseBGRATextures"));
		R += Section->Remove(TEXT("UseS3TC"));
		R += Section->Remove(TEXT("UseTNT"));
		R += Section->Remove(TEXT("UseFilterSGIS"));
		R += Section->Remove(TEXT("Use4444Textures"));
		R += Section->Remove(TEXT("UseTexPool"));
		R += Section->Remove(TEXT("UseTexIdPool"));
		R += Section->Remove(TEXT("CacheStaticMaps"));
		R += Section->Remove(TEXT("TexDXT1ToDXT3"));
		R += Section->Remove(TEXT("RequestHighResolutionZ"));
		R += Section->Remove(TEXT("NoMaskedS3TC"));
		R += Section->Remove(TEXT("DynamicTexIdRecycleLevel"));
		R += Section->Remove(TEXT("SinglePassFog"));
		R += Section->Remove(TEXT("ShaderGammaCorrectHUD"));
		R += Section->Remove(TEXT("BufferTileQuads"));
		R += Section->Remove(TEXT("BufferActorTris"));
#if !UTGLR_USES_SCENENODE_HACK
		R += Section->Remove(TEXT("SceneNodeHack"));
#endif
		R += Section->Remove(TEXT("UseFragmentProgram"));
		R += Section->Remove(TEXT("ShaderType"));
		R += Section->Remove(TEXT("SinglePassFog"));
		R += Section->Remove(TEXT("SinglePassDetail"));
		R += Section->Remove(TEXT("UseSSE"));
		R += Section->Remove(TEXT("UseSSE2"));
		R += Section->Remove(TEXT("ZRangeHack"));
		R += Section->Remove(TEXT("BufferClippedActorTris"));
		R += Section->Remove(TEXT("UseZTrick"));
		R += Section->Remove(TEXT("MaxTMUnits"));
		R += Section->Remove(TEXT("ColorizeDetailTextures"));
		R += Section->Remove(TEXT("RenderPath"));
		R += Section->Remove(TEXT("UseSamplerObjects"));
		R += Section->Remove(TEXT("NoFiltering"));
		R += Section->Remove(TEXT("ShaderMasking"));
		R += Section->Remove(TEXT("Use16BitTextures"));
...
UBOOL UOpenGLRenderDevice::Exec( const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if ( ParseCommand(&Cmd, TEXT("DGL")) )
	{
		...
		else if ( ParseCommand(&Cmd, TEXT("Remaps")) )
		{
			TexturePool.Remaps.Dump(Ar);
			return 1;
		}
		else if ( ParseCommand(&Cmd, TEXT("QuadExpansion")) )
		{
			static GLuint SavedQuadExpansion = 0;
			Exchange(FOpenGLBase::Compute.Programs.QuadExpansion, SavedQuadExpansion);
			Ar.Logf(TEXT("%s GL3 quad expansion."), SavedQuadExpansion ? TEXT("Disabled") : TEXT("Enabled"));
			return 1;
		}
		else if ( ParseCommand(&Cmd, TEXT("PaletteExpansion")) )
		{
			static GLuint SavedPaletteTextureUpdate = 0;
			Exchange(FOpenGLBase::Compute.Programs.PaletteTextureUpdate, SavedPaletteTextureUpdate);
			Ar.Logf(TEXT("%s compute palette texture update."), SavedPaletteTextureUpdate ? TEXT("Disabled") : TEXT("Enabled"));
			return 1;
		}
		else if ( ParseCommand(&Cmd, TEXT("UpdateRect")) )
		{
			SupportsUpdateTextureRect = !SupportsUpdateTextureRect;
			Ar.Logf(TEXT("%s texture update rect."), !SupportsUpdateTextureRect ? TEXT("Disabled") : TEXT("Enabled"));
			return 1;
		}
		else if ( ParseCommand(&Cmd, TEXT("MultiTile")) )
		{
			SupportsDrawTileList = !SupportsDrawTileList;
			Ar.Logf(TEXT("%s multi-tile draw."), !SupportsDrawTileList ? TEXT("Disabled") : TEXT("Enabled"));
			return 1;
		}
	}
...
void UOpenGLRenderDevice::Lock(FPlane InFlashScale, FPlane InFlashFog, FPlane ScreenClear, DWORD RenderLockFlags, BYTE* InHitData, INT* InHitSize)
{
	...
	if ( m_HitData )
	{
		...
		RenderPathName          = TEXT("HitTesting");
	}
	else if ( CurrentContextType == CONTEXTTYPE_GL1 )
	{
		...
		m_pDrawTileList         = &UOpenGLRenderDevice::DrawTileList_ARB;
		...
		m_pBlitFramebuffer      = nullptr;
		RenderPathName          = TEXT("ARB");
	}
	else if ( CurrentContextType == CONTEXTTYPE_GL3 || CurrentContextType == CONTEXTTYPE_GLES )
	{
		// No rendering
		...
		if ( GL->Compute.Programs.QuadExpansion )
		{
			m_pDrawTile             = &UOpenGLRenderDevice::DrawTileCompute_GLSL3;
			m_pDrawTileList         = &UOpenGLRenderDevice::DrawTileListCompute_GLSL3;
		}
		else
		{
			m_pDrawTile             = &UOpenGLRenderDevice::DrawTile_GLSL3;
			m_pDrawTileList         = &UOpenGLRenderDevice::DrawTileList_GLSL3;
		}
		...
		m_pBlitFramebuffer      = &UOpenGLRenderDevice::BlitFramebuffer_GLSL3;
		DrawBuffer.ActiveType = DRAWBUFFER_CommandBuffer;
		RenderPathName        = TEXT("GLSL3");
	}
	else
	{
		// No rendering
	...
	if ( (RenderLockFlags & LOCKR_ClearScreen) && (ScreenClear.X + ScreenClear.Y + ScreenClear.Z > 0) && Offscreen.IsActive() && m_pFillScreen )
	{
...
void UOpenGLRenderDevice::SetSceneNode(FSceneNode* Frame)
{
	...
	unguardf((TEXT("%s"),RenderPathName));
}
...
void UOpenGLRenderDevice::DrawTile(FSceneNode* Frame, FTextureInfo& Info, FLOAT X, FLOAT Y, FLOAT XL, FLOAT YL, FLOAT U, FLOAT V, FLOAT UL, FLOAT VL, class FSpanBuffer* Span, FLOAT Z, FPlane Color, FPlane Fog, DWORD PolyFlags)
{
	...
	UBOOL UseMaskedStorage;
	PolyFlags = FixPolyFlags(PolyFlags, UseMaskedStorage);
	bool bUnlit = !!(PolyFlags & PF_Modulated);

	PolyFlags |= PF_Gouraud;              // All tile flags buffer the color
	PolyFlags &= ~(PF_Unlit|PF_TwoSided); // These flags are irrelevant and must be removed
...
//	*(DWORD*)&Surface.Fog   = FPlaneTo_RGBAClamped(&Fog);
	...
	if ( bUnlit )
		*(DWORD*)&Surface.Color |= RGBA_MAKE(255, 255, 255, 0);
...
void UOpenGLRenderDevice::DrawTileList(const FSceneNode* Frame, const FTextureInfo& Info, const FTileRect* Tiles, INT NumTiles, FSpanBuffer* Span, FLOAT Z, FPlane Color, FPlane Fog, DWORD PolyFlags)
{
	guard(UOpenGLRenderDevice::DrawTileList);

	// Adjust PolyFlags
	UBOOL UseMaskedStorage;
	PolyFlags = FixPolyFlags(PolyFlags, UseMaskedStorage) & ~(PF_Unlit|PF_TwoSided);
	PolyFlags |= PF_Gouraud; // Always buffer color
...
//	*(DWORD*)&Surface.Fog   = FPlaneTo_RGBAClamped(&Fog);
	...
	// Render path
	if ( m_pDrawTileList )
		(this->*m_pDrawTileList)(Frame,Surface,Tiles,NumTiles,Z);
