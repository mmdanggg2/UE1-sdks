...
bool FOpenGL3::Init()
{
	...
	if ( Success )
	{
		...
		UOpenGLRenderDevice::PalettePool.Init();
	}
	...
	, ProgramID(0)
{
	...
	BindBufferBase(GL_UNIFORM_BUFFER, FGlobalRender_UBO::UniformIndex, RenDev->bufferId_GlobalRenderUBO);
	BindBufferBase(GL_UNIFORM_BUFFER, FStaticBsp_UBO::UniformIndex, RenDev->bufferId_StaticBspUBO);
	BindBufferBase(GL_UNIFORM_BUFFER, FTextureParams_UBO::UniformIndex, RenDev->bufferId_TextureParamsUBO);
	BindBufferBase(GL_UNIFORM_BUFFER, FComplexSurfaceScale_UBO::UniformIndex, RenDev->bufferId_csTextureScaleUBO);

	// Hack - find out a better way to do this (use Vulkan info?)
	bool bDisableCompute = FOpenGLBase::Vendor.InStr(TEXT("Broadcom")) != INDEX_NONE;

	// Compile compute shaders
	if ( SupportsComputeShader && !bDisableCompute )
	{
		if ( !GIsEditor )
			SetComputeQuadExpansion(0, 0);

		SetComputePaletteTextureUpdate(0, 0, 0, 0, 0, false);
	}
}
...
void FOpenGL3::Lock()
{
	...
	if ( !DrawBuffer.StreamBufferInit )
	{
		...
		DrawBuffer.ComplexStatic = new FGL::DrawBuffer::FComplexAltGLSL3;
		...
		DrawBuffer.GeneralBuffer[0] = new FGL::VertexBuffer::CopyToVBO(8 * 1024 * 1024); // Up to 60 bytes per vertex -> as little as ~140k vertices
		DrawBuffer.GeneralBuffer[1] = new FGL::VertexBuffer::CopyToVBO(2 * 1024 * 1024); // Almost enough for a DWORD index buffer of 35k quad surfs (140k vertices)
		DrawBuffer.Complex->StreamBuffer = DrawBuffer.GeneralBuffer[0];
		DrawBuffer.Complex->IndexBuffer = DrawBuffer.GeneralBuffer[1];
		DrawBuffer.ComplexStatic->StreamBuffer = DrawBuffer.GeneralBuffer[0];
		DrawBuffer.ComplexStatic->IndexBuffer = DrawBuffer.GeneralBuffer[1];
		DrawBuffer.Line->StreamBuffer = DrawBuffer.GeneralBuffer[0];
		DrawBuffer.Fill->StreamBuffer = DrawBuffer.GeneralBuffer[0]; // TODO: Remove

		if ( Compute.Programs.QuadExpansion )
			DrawBuffer.GeneralBuffer[2] = new FGL::VertexBuffer::CopyToVBO(FGL::DrawBuffer::FQuadGLSL3::MaxTileCount * 48);
	}
