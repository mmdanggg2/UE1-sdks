...
class WDlgCustomGrid : public WDialog
{
	...
	void OnInitDialog()
	{
		...
		SetText(TEXT("Custom Grid Value"));
		ValueEdit.SetText(*FString::Printf(TEXT("%d"), appRound(Max(GEditor->Constraints.GridSize.X, GEditor->Constraints.GridSize.Y, GEditor->Constraints.GridSize.Z))));
		unguard;
	}
	void OnDestroy()
	{
