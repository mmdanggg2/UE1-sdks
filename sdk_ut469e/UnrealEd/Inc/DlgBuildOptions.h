...
class WDlgBuildOptions : public WDialog
{
	...
	WLabel BSPPolysLabel{}, BSPNodesLabel{}, BSPRatioLabel{}, BSPMaxDepthLabel{}, BSPAvgDepthLabel{}, BSPPointsLabel{};
	WLabel LightLightsLabel{};
	WLabel PathPathsLabel{};
	...
	,	BSPPointsLabel		( this, IDSC_POINTS )
	,	LightLightsLabel	( this, IDSC_LIGHTS )
	,	PathPathsLabel		( this, IDSC_PATHS )
	{
	}
	...
	virtual void DoModeless(UBOOL bShow = TRUE)
	{
		...
		AddWindow(this, TEXT("WDlgBuildOptions"));
		hWnd = CreateDialogParamW( hInstance, MAKEINTRESOURCEW(IDDIALOG_BUILD_OPTIONS), OwnerWindow?OwnerWindow->hWnd:NULL, (DLGPROC)StaticDlgProc, (LPARAM)this);
		if( !hWnd )
			appGetLastError();
		Show(bShow);
		unguard;
	...
	void RefreshStats(void)
	{
		...
		GetPropResult.Empty();	GEditor->Get(TEXT("BSP"), TEXT("POINTS"), GetPropResult);	iPoints = appAtoi(*GetPropResult);
		BSPPointsLabel.SetText(*GetPropResult);
		...
		// PATHS
		PathPathsLabel.SetText( *FString::Printf(TEXT("%i"), GEditor->Level->ReachSpecs.Num()) );
