...
class WDlgScaleMap : public WDialogTool
{
	...
	virtual void DoModeless(UBOOL bShow = TRUE)
	{
		...
		hWnd = CreateDialogParamW( hInstance, MAKEINTRESOURCEW(IDDIALOG_SCALE_MAP), OwnerWindow?OwnerWindow->hWnd:NULL, (DLGPROC)StaticDlgProc, (LPARAM)this);
		if( !hWnd )
			appGetLastError();
		Show(bShow);
		unguard;
	...
	void OnOK()
	{
		...
		for (INT i = 0; i < GEditor->Level->Actors.Num(); i++)
		{
			...
			if (Actor && Actor->bSelected && !(Actor->GetFlags() & RF_TagImp))
			{
				...
				BOOL bSkipScale = !bIsBrush && (Cast<ANavigationPoint>(Actor) || Cast<AInventory>(Actor));
				BOOL bIsMovingBrush = bIsBrush && !Actor->bStatic;
				if (Checked(IDCK_LOCATIONS))
				{
					FLOAT OffsetZ = !bSkipScale ? 0.f : (Checked(IDRB_LOC_BOTTOM) ? -Actor->CollisionHeight : (Checked(IDRB_LOC_TOP) ? Actor->CollisionHeight : 0.f));
					Actor->Location.X *= X;
					Actor->Location.Y *= Y;
				...
				if (!Cast<APawn>(Actor) && !Cast<AInventory>(Actor) && !Cast<ANavigationPoint>(Actor))
				{
					...
					if (Checked(IDCK_COLLISION_HEIGHT))
						Actor->CollisionHeight *= AbsFactor[iZ];
				}
		...
		if (!Selected)
		{
			::MessageBox(hWnd, TEXT("No actors selected. Nothing to scale."), *GetText(), MB_OK);
			return;
		}
		...
		FString Report = FString::Printf(TEXT("Scaled %d actors."), Selected);
		if (OutOfMap)
			Report += FString::Printf(TEXT(" %d actors are now placed outside the level boundaries."), OutOfMap);

		::MessageBox(hWnd, *Report, *GetText(), MB_OK);
