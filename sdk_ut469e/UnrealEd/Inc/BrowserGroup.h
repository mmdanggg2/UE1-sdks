...
TBBUTTON tbBGButtons[] = {
	...
	, { 2, ID_EditDelete, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0L, 0}
	, { 0, 0, TBSTATE_ENABLED, TBSTYLE_SEP, 0L, 0}
	...
	, { 0, 0, TBSTATE_ENABLED, TBSTYLE_SEP, 0L, 0}
	, { 8, IDMN_GB_REAPPLY_VISIBILITY_SELECTED, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0L, 0}
	, { 9, IDMN_GB_REAPPLY_VISIBILITY_ALL, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0L, 0}
};
struct {
	TCHAR ToolTip[128];
	int ID;
	...
	TEXT("Reapplies Visibility Rules of Selected Group(s) to Actors in the Group(s)"), IDMN_GB_REAPPLY_VISIBILITY_SELECTED,
	TEXT("Reapplies Visibility Rules of All Group(s) to Actors in the Group(s)"), IDMN_GB_REAPPLY_VISIBILITY_ALL,
	NULL, 0
};
...
class WGroupCheckListBox : public WCheckListBox
{
	...
	{
		check(OwnerWindow);
		hbmLocked = (HBITMAP)LoadImageW(hInstance, MAKEINTRESOURCEW(IDBM_CHECKBOX_LOCKED), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);	check(hbmLocked);
		hbmUnlocked = (HBITMAP)LoadImageW(hInstance, MAKEINTRESOURCEW(IDBM_CHECKBOX_UNLOCKED), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);	check(hbmUnlocked);
		hbmVisible = (HBITMAP)LoadImageW(hInstance, MAKEINTRESOURCEW(IDBM_CHECKBOX_VISIBLE), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);	check(hbmVisible);
		hbmInvisible = (HBITMAP)LoadImageW(hInstance, MAKEINTRESOURCEW(IDBM_CHECKBOX_INVISIBLE), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);	check(hbmInvisible);
	...
	void OnDrawItem(DRAWITEMSTRUCT* Item)
	{
			...
			&& (Item->itemAction & (ODA_DRAWENTIRE | ODA_SELECT)))
		{
			...
			ExtTextOutW(Item->hDC, Item->rcItem.left + MulDiv(47, DPIX, 96),
...
class WBrowserGroup : public WBrowser, public FActorGroupCallback
{
	...
	void ShowAllGroupVisibleActors()
	{
		guard(WBrowserGroup::ShowAllGroupVisibleActors);
		GEditor->ResetZClipping();
		GEditor->Trans->Begin(TEXT("UnHide All Group-Visible"));
		GEditor->Level->Modify();

		TArray<FString> HiddenGroups;
		for (INT i = GroupList.Num() - 1; i >= 0; --i)
			if (!(GroupList(i).VisFlags & G_VISIBLE))
				HiddenGroups.AddItem(FString(TEXT(",")) + GroupList(i).GroupName + TEXT(","));
		...
		for (INT i = 0; i < GEditor->Level->Actors.Num(); i++)
		{
			...
			(	Actor
			&&	!Cast<ACamera>(Actor)
			&&	Actor != GEditor->Level->Brush()
			&&	Actor->GetClass()->GetDefaultActor()->bHiddenEd == 0 )
			{
				INT j = 0;
				if (HiddenGroups.Num() > 0)
				{
					Group = TEXT(",");
					Group += *Actor->Group;
					Group += TEXT(",");
					for (/* j */; j < HiddenGroups.Num(); j++)
						if (Group.InStr(HiddenGroups(j)) != -1)
							break;
				}
				if (j == HiddenGroups.Num())
				{
					Actor->Modify();
					Actor->bHiddenEd = 0;
				}
	...
	void NotifyLevelActor(AActor* Other, EActorAction Action)
	{
		...
		INT i;
		if (Action == NOTE_ActorSpawned)
			RefreshDelayed();
		else if (Action == NOTE_ActorDestroyed)
		{
			UBOOL bDirty = FALSE;
			for (i = (GroupList.Num() - 1); i >= 0; --i)
			{
	...
	void PositionChildControls( void )
	{
		...
		::MoveWindow(hWndToolBar, 4, 0, 14 * MulDiv(16, DPIX, 96), 14 * MulDiv(16, DPIY, 96), 1);
		RECT R;
	...
	void OnCommand( INT Command )
	{
		...
		switch( Command ) {
			...
			case IDMN_GB_REAPPLY_VISIBILITY_SELECTED:
				ReapplyGroupVisibility(0);
				break;

			case IDMN_GB_REAPPLY_VISIBILITY_ALL:
				ReapplyGroupVisibility(1);
				break;
	...
	void ReapplyGroupVisibility(UBOOL All)
	{
		guard(WBrowserGroup::ReapplyGroupVisibility);

		int SelCount = All ? pListGroups->GetCount() : pListGroups->GetSelectedCount();
		if (SelCount <= 0)
			return;
		int* Buffer = NULL;		
		if (!All)
		...
		{
			FGroupActors* G = FindByGroupName(*pListGroups->GetString(All ? s : Buffer[s]));
			if (G)
				for (TUnorderedSet<AActor*>::TIterator It(G->Actors); It; ++It)
					UpdateActorVis(It.Key(), G);
		}
