...
class WSelectGroups : public WWindow
{
	...
	void RefreshGroupsList(void)
	{
		...
		FString OldActorEntry;
		if (ActorList->GetCount())
			OldActorEntry = ActorList->GetString(ActorList->GetCurrent());

		FString OldGroupEntry;
		if (GroupList->GetCount())
			OldGroupEntry = GroupList->GetString(GroupList->GetCurrent());
		...
		if (OldActorEntry.Len())
		{
			INT UseEntry = ActorList->FindString(*OldActorEntry);
			ActorList->SetCurrent((UseEntry > 0) ? UseEntry : 0, 1);
		}
		else
		...
		if (OldGroupEntry.Len())
		{
			INT UseEntry = GroupList->FindString(*OldGroupEntry);
			GroupList->SetCurrent((UseEntry > 0) ? UseEntry : 0, 1);
		}
