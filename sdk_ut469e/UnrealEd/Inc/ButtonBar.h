...
class WButtonGroup : public WWindow
{
	...
	:	WWindow( InPersistentName, InOwnerWindow )
	{
		...
		hbmDown = (HBITMAP)LoadImageW( hInstance, MAKEINTRESOURCEW(IDBM_DOWN_ARROW), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );	check(hbmDown);
		hbmUp = (HBITMAP)LoadImageW( hInstance, MAKEINTRESOURCEW(IDBM_UP_ARROW), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );	check(hbmUp);
	...
	void AddButton( int _iID, UBOOL bAutoCheck, FString BMPFilename, FString Text, UClass* Class, FString ExecCommand = TEXT("") )
	{
		...
		if( BMPFilename.Len() )
		{
			...
			pWBButton->hbm = (HBITMAP)LoadImageW( hInstance, *Filename, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE );
			ScaleImageAndReplace(pWBButton->hbm);
			if (!pWBButton->hbm)
			{
				pWBButton->hbm = (HBITMAP)LoadImageW(hInstance, TEXT("editorres\\BBGeneric.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				ScaleImageAndReplace(pWBButton->hbm);
			}
		}
		else
					...
					switch( LastlParam ) {
						...
						case IDPB_ADD_MOVER:
						{
							...
							for( int x = 0 ; x < MoverTypes.Num() ; x++ )
							{
								...
								mif.dwTypeData = const_cast<TCHAR*>(*(pwbbmt->Name));
								mif.wID = pwbbmt->ID;
...
//			WCheckBox* pcheckbox;
		...
		if( GetDlgItem( hWnd, IDPB_CAMERA_SPEED ) )
		{
			...
			if (!hbmCamSpeed[0])
			{
				hbmCamSpeed[0] = (HBITMAP)LoadImageW( hInstance, MAKEINTRESOURCEW(IDBM_CAMSPEED1), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );	check(hbmCamSpeed[0]);
				hbmCamSpeed[1] = (HBITMAP)LoadImageW( hInstance, MAKEINTRESOURCEW(IDBM_CAMSPEED2), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );	check(hbmCamSpeed[1]);
				hbmCamSpeed[2] = (HBITMAP)LoadImageW( hInstance, MAKEINTRESOURCEW(IDBM_CAMSPEED3), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );	check(hbmCamSpeed[2]);
...
class WButtonBar : public WWindow
{
	...
	void OnCreate()
	{
		...
		pGroup->AddButton(IDPB_SHOW_ALL_GROUP_VISIBLE, 0, TEXT("ShowAllGroupVisible"), TEXT("Show All Group-Visible Actors"), NULL);
